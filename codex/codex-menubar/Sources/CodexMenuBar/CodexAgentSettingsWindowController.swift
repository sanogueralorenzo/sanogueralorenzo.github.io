  ...
  NSTableViewDelegate, NSWindowDelegate, NSSearchFieldDelegate
{
  ...
  init(
    onSave: @escaping (CodexAgentSettingsSelection) -> Void,
    onClose: @escaping () -> Void,
    onRequestNotifications: @escaping () -> Void
  ) {
    self.onSave = onSave
  ...
  private func buildUI(in panel: NSPanel) {
    ...
    notificationStatusRow.frame = NSRect(x: horizontalInset, y: 558, width: contentWidth, height: 38)
    notificationStatusRow.autoresizingMask = [.width, .minYMargin]
    contentView.addSubview(notificationStatusRow)

    let integrationsDivider = NSBox(
      frame: NSRect(x: horizontalInset, y: 532, width: contentWidth, height: 1))
    ...
    reviewTitle.frame = NSRect(x: horizontalInset, y: 500, width: contentWidth, height: 20)
    ...
    reviewModePopUp.frame = NSRect(x: horizontalInset, y: 464, width: contentWidth, height: 28)
    reviewModePopUp.autoresizingMask = [.width, .minYMargin]
    contentView.addSubview(reviewModePopUp)

    let reviewDivider = NSBox(
      frame: NSRect(x: horizontalInset, y: 436, width: contentWidth, height: 1))
    ...
    sourcePicker.frame = NSRect(x: horizontalInset, y: 400, width: contentWidth, height: 28)
    sourcePicker.autoresizingMask = [.width, .minYMargin]
    contentView.addSubview(sourcePicker)

    searchField.delegate = self
    searchField.sendsSearchStringImmediately = true
    searchField.frame = NSRect(x: horizontalInset, y: 364, width: contentWidth, height: 26)
    searchField.autoresizingMask = [.width, .minYMargin]
    contentView.addSubview(searchField)

    listHint.textColor = .secondaryLabelColor
    listHint.frame = NSRect(x: horizontalInset, y: 340, width: contentWidth, height: 18)
    listHint.autoresizingMask = [.width, .minYMargin]
    contentView.addSubview(listHint)

    configureProgressIndicator(listProgressIndicator, x: horizontalInset, y: 340)
    listProgressIndicator.autoresizingMask = [.minYMargin]
    contentView.addSubview(listProgressIndicator)

    scrollView.frame = NSRect(x: horizontalInset, y: 54, width: contentWidth, height: 278)
    ...
    allowNotificationsButton.frame = NSRect(x: 140, y: 12, width: 148, height: 30)
    allowNotificationsButton.autoresizingMask = [.minXMargin, .maxYMargin]
    allowNotificationsButton.isHidden = true
    allowNotificationsButton.isEnabled = false
