...
final class CodexCoreCLIClient: @unchecked Sendable {
  ...
  func availableProjects() throws -> [AvailableProject] {
    let output = try runAgents(["config", "available-projects", "--json"])
    let decoder = JSONDecoder()
    decoder.keyDecodingStrategy = .convertFromSnakeCase
    return try decoder.decode([AvailableProject].self, from: Data(output.utf8))
  }
  ...
  func setAllowedProjects(_ projectIDs: [Int]) throws {
    if projectIDs.isEmpty {
      _ = try runAgents(["config", "clear-allowed-projects"])
      return
    }
    _ = try runAgents(["config", "set-allowed-projects"] + projectIDs.map(String.init))
  }
  ...
  private func run(commandGroup: String, arguments: [String]) throws -> String {
    ...
    guard process.terminationStatus == 0 else {
      ...
      throw Error(
        message:
          "codex-core \(commandGroup) command failed: codex-core \(commandGroup) \(arguments.joined(separator: " "))"
      )
    }
