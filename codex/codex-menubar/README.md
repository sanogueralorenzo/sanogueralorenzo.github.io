...
- `Agents -> View` includes `Clear` as the last item when terminal runs exist; it deletes persisted completed/failed spike and task job files plus published/needs-attention review job directories from `~/.codex/agents`.
...
- `Agents -> Settings` shows integration status for `gh`, `acli`, and notification permission state, plus the review mode, allowed GitHub repositories, allowed Jira projects, and Jira-project-to-repository mappings used by agent runs.
...
- Threads menu includes:
  - `Auto-Remove` bold title row clears the saved day+mode selection.
  - `Delete Now` closes Codex only when running, executes `codex-core sessions prune --older-than-days 0 --mode delete`, then reopens Codex.
  - `1 day`, `3 days`, and `7 days` options with `Archive` and `Delete` submenus.
- Selecting a day+mode persists immediately and runs scheduled auto-remove passes hourly.
### Storage
