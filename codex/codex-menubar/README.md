...
- `Agents -> Review` keeps the open PR list and prefixes matching PR rows with bracketed persisted review job status markers: `[—]` in progress, `[X]` needs attention, `[✓]` published. PRs with no persisted review job have no prefix.
