...
- Agents commands:
  ...
  - `worker loop` starts a fresh `codex exec` run on every iteration; continuity is expected to come from your plan/prompt files, not from thread resume. Passing `--precedent-state-dir <dir>` opt-in enables Precedent: the loop uses one stable context session so injected lessons are not repeated across iterations, prepends returned context and one-shot repair blocks when present, records `diff.after_edit` when a git worktree changes, optionally records `validation.after_run` through `--precedent-validation-command`, and records `outcome.after_task` under per-iteration attempt sessions with task, scope, changed-file identity, and explicit injected-precedent attribution without failing the worker when Precedent is unavailable.
  - Important loop flags: `--prompt-file`, `--cd`, `--interval-seconds`, `--max-iterations`, `--stop-phrase`, `--once`, `--model`, `--full-auto`, `--dangerously-bypass-approvals-and-sandbox`, `--skip-git-repo-check`, `--precedent-state-dir`, `--precedent-scope`, `--precedent-changed-files`, `--precedent-validation-command`, `--precedent-validation-timeout-ms`, `--precedent-bin`.
  ...
  - `spike run <ticket> --json` loads the Jira issue through `acli jira`, also loads the latest Jira comments for context, resolves its mapped GitHub repository from `project_repo_mappings`, updates cached `main`, creates a disposable worktree under `~/.codex/agents/spikes/worktrees`, and runs `codex exec` in read-only investigation mode. The spike output contract is JSON with `summary`, `comment`, and optional `reason`; when `comment` is `null`, `reason` must be `already_covered`. Jira comments are only posted for net-new, actionable information, and when they are posted `codex exec` returns the Jira ADF `doc` object directly for pass-through posting. Spike summaries/comments and review findings now instruct Codex to format file names, classes, methods, and variables as inline code, using `file:line` only when a concrete location matters.
  - `spike jobs --json` lists persisted spike jobs from `~/.codex/agents/spikes`, including Jira URL, branch, current step, summary, and terminal error.
  ...
  - `review run <pr-url|owner/repo#number>` reuses a cached repo under `~/.codex/agents/repos/<owner>/<repo>`, creates a per-run worktree under `~/.codex/agents/worktrees`, fetches upstream review prompts from `openai/codex` `main`, appends the current PR's existing top-level comments and non-empty review bodies as one `Existing PR comments` block so `codex exec` can avoid repeating prior feedback, asks Codex to format file names, classes, methods, and variables as inline code, using `file:line` only when a concrete location matters, validates findings against changed diff lines on both left and right sides, and supports `--publish-mode publish|pending` (defaulting to `config.review_mode`).
