...
- Agents commands:
  ...
  - `review run <pr-url|owner/repo#number>` checks out the PR into `/tmp`, fetches upstream review prompts from `openai/codex` `main`, runs `codex exec` with those prompts unchanged, validates findings against changed lines, and posts inline GitHub review comments via `gh`.
