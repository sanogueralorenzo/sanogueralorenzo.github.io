...
- Agents commands:
  ...
  - `review run <pr-url|owner/repo#number>` reuses a cached repo under `~/.codex/agents/repos/<owner>/<repo>`, creates a per-run worktree under `~/.codex/agents/worktrees`, fetches upstream review prompts from `openai/codex` `main`, appends the current PR's existing top-level comments and non-empty review bodies so `codex exec` can avoid repeating prior feedback, validates findings against changed diff lines on both left and right sides, and supports `--publish-mode publish|pending` (defaulting to `config.review_mode`).
