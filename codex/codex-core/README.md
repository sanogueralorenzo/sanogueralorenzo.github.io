...
- Agents commands:
  ...
  - `review jobs --json` lists persisted review jobs from `~/.codex/agents/reviews` and includes a derived `menu_state` (`published`, `needs_attention`, `in_progress`) for clients.
  - `review show <review-id> --json` shows one persisted review job snapshot, including the derived `menu_state`.
