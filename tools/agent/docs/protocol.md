...
The runtime continues the latest Agent session across clients while it has activity within four hours. After four idle hours, or when a client sends `fresh`, the next turn creates a separate Agent session and transcript. Codex thread rollover during context compaction keeps the existing Agent session and transcript.
