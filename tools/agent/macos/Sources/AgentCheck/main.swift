...
struct AgentCheck {
    ...
    static func main() async throws {
        ...
        MockURLProtocol.handler = { request, protocolValue in
            check(request.url?.path == "/v1/events" && request.url?.query == "after=7", "Agent event cursor URL check failed")
            protocolValue.respond("""
            data: {"runId":"r1","sequence":1,"event":{"type":"turn","text":"hello","channel":"cli","hasAttachments":false}}

            data: {"runId":"r1","sequence":2,"event":{"type":"done","sessionId":"s1"}}

            """, stream: true)
        }
        let completed = try await collectRun(try await client().events(after: 7))
        ...
        do {
            _ = try await collectRun(try await client().events(after: 0))
        ...
        MockURLProtocol.handler = { _, protocolValue in
            protocolValue.respond("data: {\"runId\":\"r1\",\"sequence\":1,\"event\":{\"type\":\"status\",\"message\":\"Working\"}}\n\n", stream: true)
        }
        do {
            for try await _ in try await client().events(after: 0) {}
        ...
        MockURLProtocol.handler = { request, protocolValue in
            ...
            case ("GET", "/v1/runs"):
                protocolValue.respond(#"{"active":null,"latestSequence":4}"#)
            case ("POST", "/v1/runs"):
                let busy = submissions.withLock { count in
                    count += 1
                    return count > 1
                }
                protocolValue.respond(busy ? #"{"error":"busy"}"# : #"{"run":{"id":"r1","origin":"macos","startSequence":5}}"#, status: busy ? 409 : 202)
        ...
        let state = try await apiClient.runState()
        check(state.latestSequence == 4 && state.active == nil, "Agent run-state check failed")
        ...
        MockURLProtocol.handler = { _, protocolValue in
            protocolValue.respond("data: {\"runId\":\"r1\",\"sequence\":1,\"event\":{\"type\":\"turn\",\"text\":\"hello\",\"channel\":\"api\",\"hasAttachments\":false}}\n\n", stream: true, finish: false)
        }
        MockURLProtocol.stopped = { stopped.withLock { $0 = true } }
        var stream: AsyncThrowingStream<RunEnvelope, Error>? = try await client().events(after: 0)
