...
final class RuntimeLauncher {
    ...
    private func launch() throws {
        let task = Process()
        if let executable = ProcessInfo.processInfo.environment["AGENT_EXECUTABLE"] {
            task.executableURL = URL(filePath: executable)
            task.arguments = ["serve"]
        } else if let executable = ["/opt/homebrew/bin/agent", "/usr/local/bin/agent"].first(where: FileManager.default.isExecutableFile(atPath:)) {
            task.executableURL = URL(filePath: executable)
            task.arguments = ["serve"]
    ...
    private func findSourceRoot() -> URL? {
        ...
        for _ in 0..<5 {
            ...
            if let data = try? Data(contentsOf: package),
               String(decoding: data, as: UTF8.self).contains("\"name\": \"agent\"") {
                return current
            }
