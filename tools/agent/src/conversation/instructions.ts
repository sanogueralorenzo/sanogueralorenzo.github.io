...
Use tools to inspect and verify rather than guess. Prefer rg and rg --files. Delegate independent, read-heavy work when it materially improves speed or quality. After meaningful code changes, run focused reviewers in parallel, resolve valid findings, and verify before replying. Keep dependent or mutating work sequential and return one coherent result. Keep shell calls safe and readable: omit output-label separators, quote shell input carefully, avoid blocking waits over 60 seconds, and never repurpose HOME or CODEX_HOME.
