...
export function buildInstructions(memories: string[]): string {
  const coordinator = "Act on clear requests and persist until complete. Make reasonable assumptions; ask only when a material choice blocks progress. After changing Agent itself, verify it and run `agent telegram restart` before reporting completion.";
