...
export function renderSettings(state) {
  ...
  const items = state.setupStatus?.configured
    ...
    : `<button data-action="open-login">Log In…</button>`;
...
export function renderSessionMenu(state) {
  ...
  return `<div class="settings-backdrop" data-action="close-sessions"><section class="settings-menu session-menu" role="menu" aria-label="Recent sessions" data-menu="sessions"><strong>Recent sessions</strong>${state.sessions.length ? state.sessions.map((session) => `<button data-action="open-session" data-session="${escapeHTML(session.id)}"><span>${escapeHTML(session.title || "Untitled session")}</span>${session.activeRunId ? '<i class="spinner"></i>' : ""}</button>`).join("") : '<p class="no-sessions">No sessions yet</p>'}</section></div>`;
}
