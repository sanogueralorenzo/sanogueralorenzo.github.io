...
export function renderHeader(state) {
  ...
  return `<header class="topbar"><div class="topbar-side">${isHome ? "" : `<button class="icon-button back-button" aria-label="Back to Home" title="Home" data-action="home">${icon("back", 19)}</button>`}</div><div class="brand">${isHome ? `<span class="brand-mark">${icon("sparkle", 14)}</span><strong>Agent</strong>` : `<strong>${escapeHTML(state.selectedSessionTitle || "Conversation")}</strong>`}</div><div class="topbar-side topbar-end"><button class="icon-button settings-trigger" aria-label="Settings" title="Settings" data-action="toggle-settings">${icon("settings", 17)}</button></div></header>`;
}
