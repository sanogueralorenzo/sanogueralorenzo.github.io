...
export function renderHeader(state) {
  ...
  return `<header class="topbar"><div class="topbar-side"><button class="icon-button navigation-trigger" aria-label="Open chat navigation" title="Chats" data-action="toggle-navigation">${icon("menu", 19)}</button></div><div class="brand">${isHome ? `<span class="brand-mark">${icon("sparkle", 14)}</span><strong>Agent</strong>` : `<strong>${escapeHTML(state.sessions.find((session) => session.id === state.selectedSessionId)?.title || "Side chat")}</strong>`}</div><div class="topbar-side topbar-end">${isHome ? `<button class="icon-button new-session-button" aria-label="New session" title="New session" data-action="new-session">${icon("compose", 14)}</button>` : ""}<button class="icon-button settings-trigger" aria-label="Settings" title="Settings" data-action="toggle-settings">${icon("settings", 17)}</button></div></header>`;
}
