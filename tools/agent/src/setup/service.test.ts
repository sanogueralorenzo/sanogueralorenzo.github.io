...
describe("BackendSetupService", () => {
  it("reports the ChatGPT plan and available included usage", async () => {
    ...
    const codex = new CodexAppServer({
      ...
      args: [fixture],
      installed: true,
      env: { ...process.env, AGENT_FAKE_SCENARIO: "normal" },
    });
  ...
  it("preserves explicitly selected API-key mode when Codex is unavailable", async () => {
    const homeDir = mkdtempSync(join(tmpdir(), "agent-setup-explicit-api-"));
    paths.push(homeDir);
    const store = new Store(homeDir);
  ...
  it("does not select a saved API key unless the user explicitly chooses it", async () => {
    const homeDir = mkdtempSync(join(tmpdir(), "agent-setup-unselected-api-"));
    paths.push(homeDir);
    const store = new Store(homeDir);
    const codex = new CodexAppServer({ command: "missing-codex", installed: false });
    const service = new BackendSetupService(store, model(true), codex, () => undefined);
  ...
  it("reports a selected Codex backend as disconnected when its login has expired", async () => {
    const homeDir = mkdtempSync(join(tmpdir(), "agent-setup-expired-"));
    paths.push(homeDir);
    const store = new Store(homeDir);
    ...
    const codex = new CodexAppServer({
      ...
      args: [fixture],
      installed: true,
      env: { ...process.env, AGENT_FAKE_SCENARIO: "expired" },
    });
    const service = new BackendSetupService(store, model(false), codex, () => undefined);
  ...
  it("requires the current Codex usage API", async () => {
    const homeDir = mkdtempSync(join(tmpdir(), "agent-setup-current-protocol-"));
    paths.push(homeDir);
    const store = new Store(homeDir);
    store.setSetting("backend", "codex");
    const codex = new CodexAppServer({
      ...
      args: [fixture],
      installed: true,
      env: { ...process.env, AGENT_FAKE_SCENARIO: "missing-rate-limits" },
    });
    const service = new BackendSetupService(store, model(false), codex, () => undefined);
  ...
  it("switches to a saved API key only when explicitly selected", async () => {
    const homeDir = mkdtempSync(join(tmpdir(), "agent-setup-exhausted-"));
    paths.push(homeDir);
    const store = new Store(homeDir);
    store.setSetting("backend", "codex");
    const codex = new CodexAppServer({
      ...
      args: [fixture],
      installed: true,
      env: { ...process.env, AGENT_FAKE_SCENARIO: "exhausted" },
    });
    const service = new BackendSetupService(store, model(true), codex, () => undefined);
    const status = await service.status();
