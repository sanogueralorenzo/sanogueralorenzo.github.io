...
On macOS, setup installs and starts a private user LaunchAgent shown as **A1R Gateway** in Login Items, prints `A1R Telegram is running in the background.`, and returns the shell prompt. The gateway starts again at login and is restarted automatically if it fails; logs stay under `~/.a1r/telegram.log`. Running `a1r telegram` repairs or restarts that background service. There is no foreground gateway mode or hidden CLI service command.
