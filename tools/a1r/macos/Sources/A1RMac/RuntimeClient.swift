...
enum RuntimeClientError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notRunning: "A1R runtime is not running."
        case .incompatible: "This A1R runtime uses an incompatible protocol."
...
struct RuntimeClient: Sendable {
    ...
    static func discover() throws -> RuntimeClient {
        let homeDirectory: URL
        if let configured = ProcessInfo.processInfo.environment["A1R_HOME"], !configured.isEmpty {
            ...
            homeDirectory = FileManager.default.homeDirectoryForCurrentUser.appending(path: ".a1r")
        }
