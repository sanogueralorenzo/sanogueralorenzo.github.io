...
Home accepts overlapping requests immediately. The Pi coordinator routes from recent saved task previews and looks up older conversations only when needed. It chooses a saved conversation or creates one or more new conversations; independent outcomes get separate clickable Home entries. Follow-ups to the same task reuse its entry, while separate work in a saved conversation gets a new entry linked to that session. A conversation accepts queued follow-ups and explicit steering, with one active turn at a time. Every result returns to its Home entry. On restart, an in-flight turn is marked interrupted and can be continued manually from Home. Queued work survives the restart.
