...
Use `service/install.sh` to start Assistant after login on macOS or Linux, and `service/uninstall.sh` to remove it. State lives under `~/.assistant`. Run `npm run check` for the TypeScript check.
