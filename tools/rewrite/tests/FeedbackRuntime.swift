    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        Task { @MainActor in
            do {
                ...
                precondition(!views.contains { ($0 as? NSTextField)?.stringValue == "Shortcut" }, "no shortcut customization")
                ...
                let codes = [kVK_ANSI_1, kVK_ANSI_2, kVK_ANSI_3]
                for index in 0..<3 {
                ...
                precondition(choices == [.shorter, .clearer, .grammar], "Option-1 through Option-3 choose shorter, clearer, grammar")
                if CommandLine.arguments.contains("--menu") {
                    ...
                    print("Choose Make shorter with Option-1 in the menu now."); fflush(stdout)
                    let picked = actions.menu.popUp(positioning: actions.menu.items.first, at: NSPoint(x: 300, y: 600), in: nil)
                    precondition(picked && choices == [.shorter], "live popup Option-1 dispatch")
                    window.orderOut(nil)
                }
                shortcut.onPress = { pressed += 1 }
                precondition(shortcut.register(), "Option-R registration (quit any running Rewrite before this test)")
                let duplicate = GlobalShortcut()
                precondition(!duplicate.register(), "Conflicting hotkey rejected")
                var event: EventRef?
                ...
                print("PASS fixed-model settings/migration, busy badge, menu status/cancel, window-free errors, Option-1 through Option-3, fixed Option-R registration/conflict/callback")
                NSApp.terminate(nil)
