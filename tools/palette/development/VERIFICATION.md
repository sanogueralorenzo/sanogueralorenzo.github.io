...
## Option–Shift–V and clipboard-shaped empty guidance

Changed the sole global shortcut to Option–Shift–V, including registration, tooltip, conflict message, and README. Removed the footer label and its dynamic copy/preview hint logic. Empty history now shows three inert rows through the real clip row builder: “Copied items appear here”, “Hover + Space to preview images”, and “⌘ + number to copy”. Real results keep their per-row numbered shortcuts. No-match searches show only “No matching clips”; guidance never enters history or numbered-copy targets.
