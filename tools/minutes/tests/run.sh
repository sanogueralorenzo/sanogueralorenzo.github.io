...
cd "$minutes_root"
swiftc -parse-as-library -target "$(uname -m)-apple-macos15.0" Sources/Meeting.swift Sources/Recorder.swift tests/MinutesTests.swift \
  -framework AppKit -framework AVFoundation -framework ScreenCaptureKit -o "$stage/tests"
"$stage/tests"
