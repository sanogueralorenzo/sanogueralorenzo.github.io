...
`./tests/run.sh` checks provider migration and immediate saving, activity permissions, meeting persistence/recovery/deletion, on-demand exports (including cleared notes), real worker success/failure/cancellation, readable PCM files and lossless 60-second audio rotation, plus Python pipeline checks:

- Both fixed providers use private access-only credentials, disabled tools/context, fresh sessions, and one process per job. Sessions reset at startup and after each result; failed requests immediately close the process.
...
- Raw transcript input and explicit partial-note synthesis retain strict validation of all five note fields. The app and quality script use the same generation pipeline. Worker success and failure use one result file, and a competing worker cannot overwrite it.
