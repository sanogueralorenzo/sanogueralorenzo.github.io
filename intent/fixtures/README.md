...
- `invalid_capability_grant_argument_type.intent`: declares `run command: 30` with a non-string shell command grant argument, which should fail `INTENT_CAPABILITY_GRANT_INVALID`.
...
- `invalid_context_argument_type.intent`: declares `context web(url: 30)` with a non-string constrained URL argument, which should fail `INTENT_CONTEXT_ARGUMENT_INVALID`.
