...
- A step input bound to an earlier step output creates a `data` edge from the
  ...
  output type, and `targetSpan` for the step parameter. These values must match
  the producer type/span and target input name/type/span.
- A step input node creates a `requires` edge to its owning step, so execution
  ...
  required step parameter. These values must match the source input name/type,
  source span, and owning step.
...
- Graph requires-edge payload validation emits
  ...
  step-input attachment endpoints remain `INTENT_GRAPH_STEP_ATTACHMENT_INVALID`.
  Role-valid typed mismatches emit `INTENT_GRAPH_TYPED_EDGE_INVALID`.
...
- Graph requires-edge payload validation emits
  ...
  remain `INTENT_GRAPH_STEP_ATTACHMENT_INVALID`. Role-valid requirement or
  owner-step mismatches emit `INTENT_GRAPH_TYPED_EDGE_INVALID`.
...
- `INTENT_GRAPH_TYPED_EDGE_INVALID`
...
capability ownership, or `Capability` to `Effect`, `Check`, or `Context` for
runtime authorization. `authorizes` edges to unsupported target roles, and
non-Capability authorization edges, make graph output
non-executable. This generic role diagnostic is separate from
`INTENT_GRAPH_CAPABILITY_AUTHORIZES_INVALID`,
`INTENT_GRAPH_AUTHORIZATION_INVALID`,
`INTENT_GRAPH_AUTHORIZATION_GRANT_INVALID`, and malformed node diagnostics,
and prevents `authorizes` from becoming an ambiguous catch-all edge during
...
`INTENT_GRAPH_STEP_SEQUENCE_INVALID`,
`INTENT_GRAPH_EDGE_PAYLOAD_INVALID`, `INTENT_GRAPH_TYPED_EDGE_INVALID`, and
malformed node payload diagnostics. Runtime `data` and `supplies` edge payloads
must also match endpoint names, endpoint types, ownership, and source/target
spans. These contracts prevent topology/data edges from being replayed as
ambiguous runtime-control edges while preserving ownership, sequencing,
typed-binding, and payload diagnostics.
...
step, emits
`INTENT_GRAPH_EDGE_PAYLOAD_INVALID` when a `supplies` or `data` edge payload
omits non-empty `parameter` or `type` values or valid `sourceSpan` or
`targetSpan` values, or when a final-step-to-completion `produces` edge payload
omits non-empty `type` or valid `sourceSpan` or `targetSpan` values, or when a
step attachment edge payload omits non-empty `data.approval`, `data.policy`, or
`data.checkpoint` values,
emits `INTENT_GRAPH_TYPED_EDGE_INVALID` when role-valid `supplies`, `data`,
`requires`, `produces`, `approves`, `timeouts`, `retries`, or `checkpoints`
edge payloads do not match endpoint names, types, ownership, or source spans,
emits
...
`INTENT_GRAPH_AUTHORIZATION_INVALID`, and grant mismatches emit
`INTENT_GRAPH_AUTHORIZATION_GRANT_INVALID`.
...
`Completion` payloads remain `INTENT_GRAPH_COMPLETION_INVALID`. Role-valid
`gates` and `verifies` edge payloads must carry non-empty `data.requirement`,
valid `data.scope`, and valid source/target spans matching the source check and
owning target; malformed payloads emit `INTENT_GRAPH_EDGE_PAYLOAD_INVALID`, and
typed endpoint mismatches emit `INTENT_GRAPH_TYPED_EDGE_INVALID`. Graph
validation emits
...
external `Context` targets must be backed by matching grant records, and
...
mismatches emit `INTENT_GRAPH_AUTHORIZATION_GRANT_INVALID`.
...
`INTENT_GRAPH_GATE_INVALID` or `INTENT_GRAPH_VERIFY_INVALID`. Malformed
role-valid `gates` and `verifies` payloads emit
`INTENT_GRAPH_EDGE_PAYLOAD_INVALID`, and typed endpoint mismatches emit
`INTENT_GRAPH_TYPED_EDGE_INVALID`.
...
`INTENT_GRAPH_COMPLETION_INVALID`, `INTENT_GRAPH_GOAL_COMPLETION_INVALID`, and
`INTENT_GRAPH_EDGE_PAYLOAD_INVALID` and `INTENT_GRAPH_TYPED_EDGE_INVALID`,
...
`INTENT_GRAPH_CHECK_INVALID`; and malformed `Completion` payloads remain
`INTENT_GRAPH_COMPLETION_INVALID`. Role-valid `gates` and `verifies` payloads
must match source check requirement/scope/spans and owning target spans;
malformed payloads emit `INTENT_GRAPH_EDGE_PAYLOAD_INVALID`, and typed endpoint
mismatches emit `INTENT_GRAPH_TYPED_EDGE_INVALID`. These generic role
diagnostics prevent verification edges from becoming ambiguous runtime-control
edges while preserving check-specific gate coverage diagnostics. Graph validation emits
`INTENT_GRAPH_INVARIANT_CONSTRAINT_INVALID` when an `Invariant` node lacks
exactly one outgoing role-valid `constrains` edge to its owning `Goal`, has
duplicate role-valid `constrains` edges, or has a role-valid `constrains` edge
to the wrong `Goal`. Graph validation emits
