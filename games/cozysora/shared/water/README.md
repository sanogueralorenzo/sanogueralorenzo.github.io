...
The material translates six normal-only capillary/gravity octaves, wavelength/derivative filtering, air/water Fresnel (F0 0.02037), Beer–Lambert absorption, crest scattering, fine satin detail and a bounded, footprint-filtered GGX sunlight lobe. Small phase advection and normal-variance roughness keep the sheltered profiles from producing ruled ripple patterns or unresolved sparkling. Godot supplies the current map’s filtered sky radiance, actual lights and shadows. Sky and water direction uniforms both come from `CozyAtmosphere.DirectionToSun`, which reads the installed map sun after its authored transform; there is no map-specific fallback direction. The material uses the map’s depth-fog color, range and curve through Godot’s final fog stage. The dielectric adapter uses `SPECULAR = sqrt(0.02037 / 0.16)` to match Forward+’s squared reflectance mapping. Daan retains its existing screen-space reflections.
