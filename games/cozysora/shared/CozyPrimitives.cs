...
public static class CozyPrimitives
{
    ...
    public static MeshInstance3D Box(Node3D parent, Vector3 position, Vector3 size, Material material, bool solid = false)
    {
        var node = Instance(parent, BoxMesh(size), position, material);
        if (solid) CozyCollision.StaticBox(parent, position, size);
        return node;
    }
