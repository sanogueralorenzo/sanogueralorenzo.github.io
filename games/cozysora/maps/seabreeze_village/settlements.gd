...
var finishes = preload("res://maps/seabreeze_village/finishes.gd").new()
...
func build(world: Node3D) -> void:
	...
	railway()
	await get_tree().process_frame
	CozyMeshBatches.merge_static(self, train_cars + butterflies)


func m(hex: String) -> StandardMaterial3D:
	return w.palette.color(hex)


func b(p: Node3D, pos: Vector3, size: Vector3, color: Variant, collision := false) -> MeshInstance3D:
	return CozyPrimitives.box(p, pos, size, m(color) if color is String else color, collision)


func c(p: Node3D, pos: Vector3, r: float, h: float, color: String, top := -1.0) -> MeshInstance3D:
	return CozyPrimitives.cylinder(p, pos, r, r if top < 0 else top, h, m(color))
...
func line(p: Node3D, a: Vector3, q: Vector3, thickness: float, color: String) -> void:
	if color == "#79573d":
		...
		if thickness <= .18 and length > 3:
			b(frame, Vector3.ZERO, Vector3(thickness * 1.8, length, .16), finishes.surface("paint", color))
			for side in [-1, 1]:
				b(
					frame,
					Vector3(side * thickness * .82, 0, 0),
					Vector3(.065, length, .11),
					finishes.surface("paint", color)
				)
			...
			b(frame, Vector3.ZERO, Vector3(thickness * 2, length, thickness * 2), finishes.surface("paint", color))
	else:
...
func roof(
	...
	if hip:
		...
		for sx in [-1, 1]:
			triangle(
				mesh, Vector3(sx * half, 0, -depth * .5), Vector3(sx * half, 0, depth * .5), Vector3(sx * half, rise, 0)
			)
	mesh.generate_normals()
	var roofmesh := MeshInstance3D.new()
	roofmesh.mesh = mesh.commit()
	var mat := finishes.surface("tile", tint)
...
func house(
	...
	b(
		...
		finishes.surface("plaster" if kind == "stone" else kind, tint),
		true
	)
	b(n, Vector3(0, .17, 0), Vector3(width + .2, .35, depth + .2), finishes.surface("stone", "#8f8b7d"))
	for sx in [-1, 1]:
		for sz in [-1, 1]:
	...
	for floor_no in floors:
		var fy := floor_no * 2.9
		if floor_no > 0:
			b(n, Vector3(0, fy, 0), Vector3(width + .08, .18, depth + .08), "#493a2e")
		for side in [-1, 1]:
			var wall := Node3D.new()
			n.add_child(wall)
		...
		if floor_no == 1 and balcony:
			...
			for i in range(int((width - .5) / .23)):
				b(
					n,
					Vector3(-width * .5 + .3 + i * .23, fy + .51, depth * .5 + .95),
					Vector3(.035, .83, .035),
					"#625c4f"
				)
			for panel in 3:
				...
				b(
					n,
					Vector3(wx, 4.85, depth * .5 + .92),
					Vector3(blind_width, 1.85, .025),
					finishes.surface("bamboo", "#817761")
				)
	...
	if absf(x + 10.5) < .01 and absf(z - 84) < .01:
		roof(n, Vector3(0, h + .05, 0), depth + 1.2, width + 1.2, (width + 1.2) * .34, "#3f484b", false, 0).rotation.y = (
			PI / 2
		)
		...
		for side in [-1, 1]:
			triangle(
				gable,
				Vector3(-(width + 1.18) * .5, h + .05, side * (depth * .5 + .605)),
				Vector3((width + 1.18) * .5, h + .05, side * (depth * .5 + .605)),
				Vector3(0, h + .05 + (width + 1.2) * .34, side * (depth * .5 + .605))
			)
		...
		roof(
			...
			depth + 1.2,
			depth * .34,
			roof_color if not roof_color.is_empty() else ("#596366" if tint != "#d9d4bf" else "#a6aaa4"),
			hip,
			.3,
			"#554330" if not roof_color.is_empty() else ""
		)
...
func farm() -> void:
	...
	b(kura, Vector3(0, 3.8, 0), Vector3(4.8, 5.6, 4.4), finishes.surface("wood", "#885f37"), true)
	...
	for xx in range(47):
		for sz in [-1, 1]:
			b(kura, Vector3(-2.3 + xx * .1, 3.8, sz * 2.207), Vector3(.009, 4.4, .012), "#5b3d27")
	for y in [2.9, 4.3, 5.8]:
		b(kura, Vector3(0, y, 0), Vector3(4.9, .10, 4.5), "#554635")
	b(kura, Vector3(0, 3.25, 2.24), Vector3(2.2, 2.8, .10), finishes.surface("wood", "#ad8f66"))
	for sx in [-1, 1]:
		b(kura, Vector3(sx * 1.45, 5.7, 2.24), Vector3(.7, .5, .1), "#282b23")
	...
	for sx in [-1, 1]:
		b(awning, Vector3(sx * 3.5, .98, -2.75), Vector3(.16, 1.95, .16), "#4a3826")
...
func village() -> void:
	...
	b(self, Vector3(-10, street_y + .015, 77), Vector3(30, .012, 3.7), finishes.surface("asphalt", "#5e5754"))
	for z in [70.9, 79.15]:
		b(self, Vector3(-11, street_y + .02, z), Vector3(48, .16, .16), "#a7a18b")
	for x in [-14.2, -9.8, -3, 1.6]:
		b(self, Vector3(x, street_y + .027, 75.15), Vector3(.55, .015, .34), "#273b3c")
		for i in 7:
			b(self, Vector3(x - .23 + i * .077, street_y + .04, 75.15), Vector3(.014, .015, .31), "#697772")
	for data in [
		[-35, 82.5, 6.5, 6.5, "#a8aab0", "metal", PI, false],
		[-20.5, 84, 7.5, 7, "#70767c", "metal", -PI / 2, false],
		[-10.5, 84, 8.5, 7, "#b4bfc2", "metal", PI, false],
		[-1, 83.5, 8, 6.5, "#9aa3a8", "metal", PI, false],
		[8.5, 84, 8, 7, "#846951", "wood", PI, true]
	]:
		house(data[0], data[1], data[2], data[3], data[4], data[5], data[6], true, data[7])
	for data in [
		[-22, 59.5, 9, 7, "#977b56", "wood"],
		[-31.5, 55.5, 8, 7, "#a8aab0", "metal"],
		[-12.5, 58.4, 8, 6, "#d9d4bf", "stone"],
		[-3.5, 58, 7, 6, "#d9d4bf", "stone"],
		[-38, 92, 9, 7, "#826d4e", "wood"],
		[-8, 92.5, 9, 7, "#8a8e8c", "metal"],
		[4, 91, 9, 7, "#c9c5b5", "stone"]
	]:
	...
	for d in [
		[-20.5, 79.9, 7.1, "#776651", "SAKE"],
		[-10.5, 79.95, 7.8, "#8a9298", "COZY STORE"],
		[-1, 79.8, 6.8, "#557578", "SORA"]
	]:
		...
		for i in [-1, 1]:
			var lantern: MeshInstance3D = CozyPrimitives.sphere(
				n, Vector3(i * float(d[2]) * .42, 2.9, .45), Vector3(.24, .39, .24), m("#d45537")
			)
	...
	for d in [[-28, 73.2], [-27.2, 86.9], [-17, 74], [-7, 74], [2, 73.5]]:
		var n := group(d[0], d[1])
		for j in 3:
			crate(n, Vector3(j * .53, 0, 0), "#4d6967")
	...
	for d in [[-12.4, 74.4], [-15.2, 75], [.5, 75.3]]:
		cone(d[0], d[1])
...
func shrine() -> void:
	...
	b(n, Vector3(0, .1, 0), Vector3(6.6, .2, 6.4), finishes.surface("stone", "#d3cdbb"), true)
	b(n, Vector3(0, .42, 0), Vector3(5.8, .85, 5.6), finishes.surface("stone", "#c7c1ab"), true)
	b(n, Vector3(0, 2.2, 0), Vector3(4.8, 2.7, 4.6), "#beb9a5", true)
	for sx in [-1, 1]:
		for j in 5:
			b(n, Vector3(-2.4 + j * 1.2, 2.2, sx * 2.3), Vector3(.14, 2.7, .16), "#58472e")
		for j in 4:
			b(n, Vector3(sx * 2.4, 2.2, -2.3 + j * 1.533), Vector3(.16, 2.7, .14), "#58472e")
	for y in [.87, 2.4, 3.54]:
		b(n, Vector3(0, y, 0), Vector3(4.9, .16, 4.7), "#58472e")
	b(n, Vector3(0, 1.63, 2.33), Vector3(4.3, 1.47, .08), finishes.surface("wood", "#705339"))
	for i in 21:
		b(n, Vector3(-2.05 + i * .205, 1.63, 2.40), Vector3(.035, 1.4, .04), "#5a4934")
	...
	for d in [[-4.2, 24.5], [5.4, 27]]:
		...
		for sx in [-1, 1]:
			b(lantern, Vector3(0, 1.74, sx * .214), Vector3(.24, .24, .02), "#42483c")
		roof(lantern, Vector3(0, 2, 0), .74, .74, .32, "#929989", true)
		CozyPrimitives.sphere(lantern, Vector3(0, 2.45, 0), Vector3(.1, .1, .1), m("#a1a491"))
...
func paddies() -> void:
	...
	for area in [Rect2(63.5, -8, 35.5, 52), Rect2(42, 0, 10, 30)]:
		...
		canopy_material.shader = load("res://maps/seabreeze_village/rice_canopy.gdshader")
		...
		for j in 3:
			...
			for section in 4:
				...
				var points: Array[Vector3] = [
					lower - side * width * (1 - t0) * .5,
					lower + side * width * (1 - t0) * .5,
					upper + side * width * (1 - t1) * .5,
					lower - side * width * (1 - t0) * .5,
					upper + side * width * (1 - t1) * .5,
					upper - side * width * (1 - t1) * .5
				]
				for point in points:
					blades.add_vertex(point)
		blades.generate_normals()
		var rice_mat := ShaderMaterial.new()
		rice_mat.shader = load("res://maps/seabreeze_village/rice_blades.gdshader")
		...
		for ix in rows:
			for iz in cols:
				...
				if area.size.x > 20 and (absf(x - 81.2) < .33 or absf(z - 17.8) < .3):
					continue
				var scale_y := rng.randf_range(.7, .9)
				var basis := Basis(Vector3.UP, rng.randf() * TAU).scaled(Vector3(scale_y * 2.4, scale_y, scale_y * 2.4))
				transforms.append(
					Transform3D(basis, Vector3(x + rng.randf_range(-.04, .04), .42, z + rng.randf_range(-.04, .04)))
				)
		...
		add_child(inst)
		for z in [area.position.y, area.end.y]:
			b(self, Vector3(area.get_center().x, .48, z), Vector3(area.size.x + .5, .17, .4), "#858660")
		for x in [area.position.x, area.end.x]:
			b(self, Vector3(x, .48, area.get_center().y), Vector3(.4, .17, area.size.y), "#858660")
		if area.size.x > 20:
...
func fence(a: Vector2, q: Vector2, height: float, color: String, wire: bool) -> void:
	...
	for i in range(steps + 1):
		...
		b(
			self,
			ground + Vector3(0, height * .5, 0),
			Vector3(.075 if wire else .13, height, .075 if wire else .13),
			color
		)
		...
		if i > 0:
			for y in [.45, .85] if not wire else [.12, .52, .92]:
				line(
					self,
					points[i - 1] + Vector3(0, y * height, 0),
					ground + Vector3(0, y * height, 0),
					.025 if wire else .045,
					color
				)
			if wire:
	...
	CozyCollision.static_box(barrier, Vector3.ZERO, Vector3(distance, height, .09))
...
func poles(data: Array, dark := false) -> void:
	...
	for i in data.size():
		...
		if dark:
			...
			shaft.material_override = finishes.surface("pole", "#78786d")
			for rung in 9:
				b(n, Vector3(0, 2.5 + rung * .75, 0), Vector3(.05, .05, .42), "#3a3b38")
		else:
...
func vending_machine(x: float, z: float, yaw: float, tint: String, worn := false) -> Node3D:
	var n := group(x, z, yaw)
	b(n, Vector3(0, 1, 0), Vector3(1, 1.9, .8), finishes.surface("paint", tint), true)
	...
	for yy in [.87, 1.748]:
		b(n, Vector3(-.14, yy, .438), Vector3(.66, .03, .035), "#bfc6bd")
	...
	for row in 5 if worn else 3:
		for col in 11 if worn else 9:
			...
			if rng.randf() > .35:
				c(n, Vector3(xx, yy + can_height * .5 + .01, .458), .010, .024, "#e4dfc9")
	...
	label(
		n,
		"CUP" if tint == "#1f2f6c" else ("ICE" if tint == "#e8e6e0" else "GM"),
		Vector3(-.13, .56, .426),
		.13,
		"#e6e9d4"
	)
...
func vending_area() -> void:
	...
	for j in 2:
		...
		for i in 3:
			crate(n, Vector3(0, i * .33, 0), "#c84040" if (j == 0 and i == 2) or (j == 1 and i == 1) else "#3a8a4a")
	var boxes := group(-66.25, 29.4, .3)
	for i in 3:
		b(boxes, Vector3((i % 2) * .2, .20 + i * .34, 0), Vector3(.48, .37, .44), finishes.surface("wood", "#b0a17f"))
	for d in [[-65.75, 29.7], [-65.65, 27.4], [-65.6, 25], [-65.7, 21.7]]:
		cone(d[0], d[1])
	poles([[-76.5, 0, 8.4], [-76, 20.5, 8.6], [-75.5, 44, 8.6]])
	bicycle(-66.55, 22.3, -PI / 2, "#bfa345")
	var cone_points: Array[Vector2] = [
		Vector2(-65.75, 29.7), Vector2(-65.65, 27.4), Vector2(-65.6, 25), Vector2(-65.7, 21.7)
	]
	for i in 3:
		var a := cone_points[i]
...
func crate(p: Node3D, pos: Vector3, tint: String) -> void:
	...
	for y in [.05, .15, .25]:
		for sx in [-1, 1]:
			b(p, pos + Vector3(sx * .251, y, 0), Vector3(.012, .026, .30), "#243d35")
	for i in 4:
		for sz in [-1, 1]:
			b(p, pos + Vector3(-.18 + i * .12, .15, sz * .191), Vector3(.055, .17, .012), "#294d43")
...
func car(x: float, z: float, yaw: float, truck: bool) -> void:
	...
	if truck:
		...
		for side in [-1, 1]:
			b(n, Vector3(-1.05, .86, side * .76), Vector3(1.5, .28, .09), "#d1d5c7")
	else:
...
func bicycle(x: float, z: float, yaw: float, tint: String) -> void:
	...
	for pair in [
		[Vector3(-.6, .36, 0), Vector3(-.2, .8, 0)],
		[Vector3(-.2, .8, 0), Vector3(.08, .36, 0)],
		[Vector3(.08, .36, 0), Vector3(-.6, .36, 0)],
		[Vector3(-.2, .8, 0), Vector3(.46, .86, 0)],
		[Vector3(.46, .86, 0), Vector3(.08, .36, 0)],
		[Vector3(.46, .86, 0), Vector3(.6, .36, 0)]
	]:
...
func railway() -> void:
	...
	while distance < length:
		...
		if not bridge:
			b(
				segment,
				Vector3(0, -.65, 0),
				Vector3(a.distance_to(q) + .04, 1.3, 8.5),
				finishes.surface("concrete", "#b0aea0")
			)
		else:
		...
		for sz in [-1, 1]:
			if not bridge:
				...
				for y in [1.4, 1.9]:
					b(segment, Vector3(0, y, sz * 4.12), Vector3(2.05, .04, .04), "#e3e2d2")
			b(segment, Vector3(0, .31, sz * .83), Vector3(2.06, .13, .09), "#5e655f")
		for j in 3:
			b(segment, Vector3(-.7 + j * .7, .17, 0), Vector3(.19, .12, 2.25), "#6d7364")
		if int(distance / 2) % 6 == 0 and not bridge:
			...
			if ph > 1:
				b(
					segment,
					Vector3(0, -2.2 - ph * .5, 0),
					Vector3(1.65, ph, 4.9),
					finishes.surface("concrete", "#a8a799"),
					true
				)
		...
		if int(distance / 2) % 13 == 0:
			...
			for sz in [-1, 1]:
				c(segment, Vector3(0, 6.9, sz * .8), .07, .6, "#b8beb0")
		for sz in [-.7, .7]:
	...
	for sign_value in [-1, 1]:
		...
		line(
			self,
			start.lerp(finish, .125) + offset + Vector3(0, 9, 0),
			start.lerp(finish, .875) + offset + Vector3(0, 9, 0),
			.23,
			"#79573d"
		)
		for i in 8:
			...
			if i > 0:
				line(self, low, a + Vector3(0, 9, 0), .14, "#79573d")
			if i == 0:
				line(self, low, q + Vector3(0, 9, 0), .18, "#79573d")
			elif i == 7:
				line(self, a + Vector3(0, 9, 0), q - Vector3(0, .6, 0), .18, "#79573d")
			elif i < 4:
				line(self, a + Vector3(0, 9, 0), q - Vector3(0, .6, 0), .14, "#79573d")
			else:
				line(self, low, q + Vector3(0, 9, 0), .14, "#79573d")
	for i in range(1, 8):
	...
	for i in 4:
		...
		for sz in [-1, 1]:
			...
			for j in 4:
				...
				for sx in [-1, 1]:
					b(n, Vector3(dx + sx * .42, 3.12, sz * 1.505), Vector3(.65, .80, .025), "#2b4148")
...
func track_z(x: float) -> float:
	for i in range(track.point_count - 1):
		...
		if x >= a.x and x <= q.x:
			return lerpf(a.z, q.z, (x - a.x) / (q.x - a.x))
	return 96.0
...
func village_utility_yard() -> void:
	...
	for xx in [-.69, .69]:
		for zz in [-.22, .22]:
			b(rack, Vector3(xx, .925, zz), Vector3(.07, 1.85, .07), "#5a4632")
	for yy in [.07, .925, 1.8]:
		b(rack, Vector3(0, yy, 0), Vector3(1.45, .07, .5), "#816a49")
	for i in 6:
		for sz in [-1, 1]:
			b(rack, Vector3(-.59 + i * .24, .95, sz * .25), Vector3(.1, 1.63, .045), "#9d8158")
	var n := group(-8.15, 73.65, .4)
	for i in 3:
		crate(n, Vector3(0, i * .33, 0), ["#7a5a3a", "#5a4d38", "#8a7a48"][i])
	b(n, Vector3(0, 1.16, 0), Vector3(.4, .34, .36), "#b89d68")
	var pipe := group(-6.2, 73.55)
	line(pipe, Vector3(0, .62, 0), Vector3(5.2, .62, 0), .11, "#c3a743")
	for xx in [.3, 2.4]:
		c(pipe, Vector3(xx, .31, 0), .04, .62, "#8e8980")
	for xx in [.675, .925]:
	...
	for xx in [-.6, .6]:
		for zz in [-.12, .12]:
			line(barricade, Vector3(xx, 0, zz), Vector3(xx, .85, 0), .02, "#bdc3b5")
	for yy in [.42, .78]:
...
func chainlink_panel(a: Vector3, q: Vector3, height: float) -> void:
	...
	var verts: Array[Vector3] = [
		a, q, q + Vector3(0, height, 0), a, a + Vector3(0, height, 0), q + Vector3(0, height, 0)
	]
	...
	inst.mesh = st.commit()
	inst.material_override = finishes.chainlink()
	add_child(inst)
	...
	for i in range(pts.size() - 1):
		...
		if count > 0 and previous.distance_to(v) < 5:
			...
			for off in [-.105, 0, .105]:
				b(
					rail,
					Vector3(0, off, .012 if off == 0 else 0),
					Vector3(previous.distance_to(v) + .06, .085, .035),
					"#8e9578"
				)
		if count % 2 == 0:
			b(self, v - Vector3(0, .30, -.1), Vector3(.09, .675, .12), "#7a8668")
		previous = v
		count += 1
...
func rail_signs() -> void:
	...
	for pair in [
		[Vector3(-.34, .30, .085), Vector3(.34, .30, .085)],
		[Vector3(.34, .30, .085), Vector3(0, -.30, .085)],
		[Vector3(0, -.30, .085), Vector3(-.34, .30, .085)]
	]:
...
func shrine_gable(n: Node3D) -> void:
	for sz in [-1, 1]:
		...
		face.mesh = st.commit()
		var mat := finishes.surface("wood", "#c1b691")
		mat.cull_mode = BaseMaterial3D.CULL_DISABLED
...
func roof_point(
	half: float, ridge: float, depth: float, rise: float, curved: float, side: float, t: float, u: float, hip: bool
) -> Vector3:
...
func hazard_tape(a: Vector3, q: Vector3, height: float) -> void:
	...
	for i in count:
		...
		var tape := b(
			self,
			(p + end) * .5,
			Vector3(p.distance_to(end) + .003, height, .009),
			"#c8b64a" if i % 2 == 0 else "#333e36"
		)
	...
	for side in 4:
		...
		for row in maxi(1, int(height / .35)):
			...
			while x < length * .5:
				...
				st.begin(Mesh.PRIMITIVE_TRIANGLES)
				var verts: Array[Vector3] = [
					Vector3(x, y + .1, distance + .012),
					Vector3(x + sw * .28, y + .02, distance + .012),
					Vector3(minf(length * .5, x + sw * .95), y + .06, distance + .012),
					Vector3(minf(length * .5, x + sw * .87), y + .31, distance + .012),
					Vector3(x + sw * .17, y + .32, distance + .012)
				]
				for j in range(1, 4):
