extends RefCounted
## Sunny Bay's static geometry is batched by material and spatial cell.
var root: Node3D
var groups = {}
var materials = {}
var primitives = {}
...
func material(color: String, kind: String = "plaster") -> Material:
	...
	if kind == "foliage":
		m.shader = load("res://maps/sunny_bay/leaves.gdshader")
		m.set_shader_parameter("leaf_cards", true)
		if leaf_texture == null:
			leaf_texture = preload("res://maps/sunny_bay/textures.gd").leaf_spray()
		m.set_shader_parameter("leaf_texture", leaf_texture)
		m.set_shader_parameter("base_color", Color(color))
		materials[key] = m
		return m
	m.shader = load("res://maps/sunny_bay/surface.gdshader")
	m.set_shader_parameter("base_color", Color(color))
...
func ribbon(
	...
	for i in range(points.size() - 1):
		...
		var tangent_a = b - a if i == 0 else b - Vector3(points[i - 1])
		var tangent_b = b - a if i == points.size() - 2 else Vector3(points[i + 2]) - a
		var side_a = tangent_a.cross(Vector3.UP).normalized() * width * .5
		var side_b = tangent_b.cross(Vector3.UP).normalized() * width * .5
		var corners = [a - side_a, b - side_b, b + side_b, a + side_a]
...
func cloth(at: Vector3, size: Vector2, color: String, yaw: float = 0) -> void:
	...
	mat.shader = load("res://maps/sunny_bay/cloth.gdshader")
