...
func build() -> void:
	...
	if FileAccess.file_exists(cache):
		...
		if packed.pack(static_content)==OK:ResourceSaver.save(packed,cache)
...
func _own(node:Node) -> void:
	for child in node.get_children():child.owner=static_content;_own(child)
...
func height_at(x:float,z:float) -> float:
	...
	for court_x in [-40.0,52.0]:
		for court_z in [-68.0,7.0]:
			...
			if radius<5.5:return lerpf(terrain_height(court_x,court_z+1),ground,smoothstep(3.4,5.5,radius))
			for side in [-1,1]:
				var center_x=court_x+side*14
				var edge=maxf(absf(x-center_x)-4.1,absf(z-court_z)-7.0)
				if edge<2:return lerpf(terrain_height(center_x,court_z),ground,smoothstep(0,2,edge))
	var overlook=Vector2((x+110)/10.5,(z-99)/8.5).length()
	if overlook<1.3:return lerpf(terrain_height(-110,99),ground,smoothstep(1,1.3,overlook))
	if x>-120 and x<-108 and z<-119 and z>-163:return 2.65
	return terrain_height(x,z)

func walkable(x:float,z:float) -> bool:
	if x>-119.5 and x<-108.5 and z>-162 and z<-116:return true
	return absf(x)<171 and z>-116.5 and z<170

func set_paused(value:bool) -> void:
	if is_instance_valid(transit):transit.set_paused(value)
...
func _terrain() -> void:
	...
	var path=[]
	for x in range(-174,175,2):path.append(point(x,-113,.09))
	...
	for x in [-147.0,151.0]:
		path=[]
		for z in range(-104,165,2):path.append(point(x+sin(z*.018)*9,z,.05))
	...
	for rect in [Rect2(-620,-119,440,739),Rect2(180,-119,440,739),Rect2(-180,180,360,440)]:
		...
		for iz in range(ceili(rect.size.y/4)):
			for ix in range(ceili(rect.size.x/4)):
				var x=rect.position.x+ix*4;var z=rect.position.y+iz*4
				var positions=[]
				for p in [Vector2(x,z),Vector2(x+4,z),Vector2(x+4,z+4),Vector2(x,z+4)]:
					var outside=maxf(absf(p.x)-180,p.y-180)
					positions.append(Vector3(p.x,terrain_height(p.x,p.y)+smoothstep(0,110,outside)*(12+11*sin(p.x*.009)*sin(p.y*.012)),p.y))
				for i in [0,1,2,0,2,3]:st.add_vertex(positions[i])
		st.generate_normals();var n=MeshInstance3D.new();n.mesh=st.commit();n.material_override=geo.material("71866d");static_content.add_child(n)
	# Suggest adjoining neighborhoods behind the playable streets without copied buildings.
	for i in range(42):
		var x=-157+i*8.0;var z=199+(i%3)*14
		var y=terrain_height(x,z);var h=7+(i%5)*1.6
		...
		for row in range(2):
			for col in range(3):geo.box(Vector3(x-2+col*2,y+2+row*3,z-5.03),Vector3(.9,1.2,.04),"607d7e")
...
func _streets() -> void:
	for x in STREETS_X:
		...
		for z in range(-93,130,9):
			var crossing=false
			for cross in STREETS_Z:if absf(z-cross)<7:crossing=true
			if not crossing:
	...
	for dx in [-.76,.76,0]:
		var path=[]
		for z in range(-103,93):path.append(point(8+dx,z,.052))
...
func _fog_and_birds() -> void:
	for i in range(9):
		var fog=MeshInstance3D.new();var mesh=QuadMesh.new();mesh.size=Vector2(150+i*9,20+i*2);fog.mesh=mesh;var material=ShaderMaterial.new();material.shader=load("res://maps/harbor_hills/fog.gdshader");material.set_shader_parameter("phase",i*1.7);fog.material_override=material;fog.position=Vector3(-260+i*72,6+i%3*4,-170-i%3*75);fog.rotation.x=-.13;fog.cast_shadow=GeometryInstance3D.SHADOW_CASTING_SETTING_OFF;add_child(fog);fog_banks.append(fog)

func _process(delta:float) -> void:
	time+=delta
	for i in fog_banks.size():fog_banks[i].position.x=-260+i*72+sin(time*.013+i)*33
