...
func build(world, geometry) -> void:
	...
	for i in range(120):
		var x = rng.randf_range(-166, -87)
		var z = rng.randf_range(-30, 163)
		if absf(x + 104) < 5 and z > 43 and z < 98:
			continue
		if Vector2(x + 109, z - 96).length() < 16:
			continue
	...
	for i in range(100):
		var x = rng.randf_range(-91, -18)
		var z = rng.randf_range(57, 162)
		if absf(x + 38) < 5 and z < 98:
			continue
		if Vector2(x + 56, z - 62).length() < 11:
			continue
		if Vector2(x + 54, z - 110).length() < 15:
			continue
		if _on_path(x, z, 4):
			continue
	...
	for z in [-74., -57., -12., 24., 70.]:
		_street_tree(map.point(12.65, z), rng.randf_range(6.2, 7.5))
	...
	for route in [
		[Vector2(-112, 101), Vector2(-54, 110)],
		[Vector2(-104, 74), Vector2(-22, 111)],
		[Vector2(-55, 110), Vector2(-16, 133)]
	]:
		var path = []
		for i in range(42):
			var p: Vector2 = route[0].lerp(route[1], i / 41.0)
			path.append(map.point(p.x, p.y, .08))
		g.ribbon(path, 3.0, "aca98c", true)
	for i in range(360):
		var x = rng.randf_range(-91, -18)
		var z = rng.randf_range(54, 168)
		if absf(x + 38) < 3 and z < 98:
			continue
		if _on_path(x, z, 2):
			continue
		var p = map.point(x, z)
		...
		for j in range(3):
			...
			if not map.road_end_contains(at.x, at.z, .4):
				g.add("sphere", at, Vector3(.13, .12, .13), "d2bb7c")
	# Street trees fit the planting strip, never a doorway or the rail alignment.
	for x in [-85.0, 99.0]:
		for z in range(-82, 130, 19):
			_cypress(map.point(x, z), rng.randf_range(4.5, 6.0), z)
	for x in range(-152, 163, 19):
		if absf(x - 8) < 10:
			continue
		_cypress(map.point(x, -107), rng.randf_range(4.2, 6.4), x)
	for i in range(35):
		var x = rng.randf_range(132, 171)
		var z = rng.randf_range(-73, 164)
		_cypress(map.point(x, z), rng.randf_range(6, 10), i)
	for x in [31.0, 53.0, 75.0]:
		for z in [128.0, 154.0]:
			_street_tree(map.point(x, z), rng.randf_range(6.2, 8.2))
	# Perimeter scrub and wildflowers establish a finished edge rather than an empty plane.
	for i in range(1500):
		var x = rng.randf_range(-173, 173)
		var z = rng.randf_range(-107, 171)
		if x > -83 and x < 128 and z < 136:
			continue
		if (absf(x + 104) < 3 or absf(x + 38) < 3) and z > 44 and z < 95:
			continue
		var p = map.point(x, z)
		g.add(
			"leaf",
			p + Vector3(0, .3, 0),
			Vector3(rng.randf_range(.6, 1.5), rng.randf_range(.55, 1.3), rng.randf_range(.6, 1.5)),
			["677f5a", "89946a", "55765b", "9b9c6e"][i % 4]
		)
		if i % 3 == 0:
			for j in range(4):
				var at = p + Vector3(rng.randf_range(-.5, .5), .6 + rng.randf() * .3, rng.randf_range(-.5, .5))
				if not map.road_end_contains(at.x, at.z, .4):
					g.add("sphere", at, Vector3(.12, .12, .12), ["d3bf80", "b9979c", "ddd3a8"][i % 3])
	...
	for i in range(4):
		g.add("leaf", p + Vector3(-8 + i * .6, .6, -5), Vector3(1.1, 1.1, 1.1), "879666")
	_skyline_and_bridge()
	_boats()
	_grass()
...
func _skyline_and_bridge() -> void:
		...
		for ix in range(28):
			for iz in range(36):
				...
				for j in [0, 1, 2, 0, 2, 3]:
					st.add_vertex(positions[j])
		st.generate_normals()
		var n = MeshInstance3D.new()
		n.mesh = st.commit()
		n.material_override = g.material("788f92")
		g.root.add_child(n)
	for i in range(65):
		var x = 180 + i * 5.8
		var z = -310 + rng.randf_range(-20, 20)
		var h = rng.randf_range(9, 43)
		if i % 13 == 0:
			h *= 1.5
		g.box(Vector3(x, h * .5 - 1, z), Vector3(rng.randf_range(4, 8), h, 7), ["687f89", "7b9298", "576f7b"][i % 3])
		if i % 6 == 0:
			g.box(Vector3(x, h + 1, z), Vector3(2, 2, 3), "91a4a7")
	...
	for x in [-310.0, -195.0]:
		...
		for y in [21.0, 38.0, 54.0]:
			g.box(Vector3(x, y, z), Vector3(2.5, 1.5, 10), "a77e6c")
	for side in [-1, 1]:
		for section in [[left, -310.0], [-310.0, -195.0], [-195.0, right]]:
			var prev = Vector3.ZERO
			for i in range(33):
				var t = i / 32.0
				var x = lerpf(section[0], section[1], t)
				var height = 55 - 33 * sin(t * PI)
				if section[0] == left:
					height = lerpf(18, 55, t)
				if section[1] == right:
					height = lerpf(55, 18, t)
				var at = Vector3(x, height, z + side * 4)
				if i > 0:
					g.beam(prev, at, .18, "a78779")
				if i % 2 == 0:
					g.beam(Vector3(x, 16, z + side * 4), at, .055, "a78779")
				prev = at
...
func _boats() -> void:
	for i in range(14):
		...
		for j in range(sections.size() - 1):
			for side in [-1, 1]:
				var a: Vector3 = sections[j]
				var b: Vector3 = sections[j + 1]
				...
				for band in range(2):
					for v in [
						ring_a[band], ring_b[band], ring_b[band + 1], ring_a[band], ring_b[band + 1], ring_a[band + 1]
					]:
						st.add_vertex(v)
		st.generate_normals()
		var n = MeshInstance3D.new()
		...
		mat.albedo_color = Color(["d8d9bf", "72958b", "b56d50"][i % 3])
		...
		for side in [-1, 1]:
			...
			for section in sections.slice(1):
				var next = p + turn * Vector3(section.x * side, .62, section.z)
				g.beam(previous, next, .045, "d3c39e")
				previous = next
		...
		if i % 2 == 0:
			...
			for row in range(12):
				...
				var t1 = (row + 1) / 12.
				var corners = [
					Vector3(.07, 1.8 + t * 6.8, -.8),
					Vector3(.07 + sin(t * PI) * .24, 1.8 + t * 6.8, 2.6 - t * 3.4),
					Vector3(.07 + sin(t1 * PI) * .24, 1.8 + t1 * 6.8, 2.6 - t1 * 3.4),
					Vector3(.07, 1.8 + t1 * 6.8, -.8)
				]
				for j in [0, 1, 2, 0, 2, 3]:
					sail.add_vertex(corners[j])
...
func _grass() -> void:
	...
	for i in range(5):
		...
		var h = .52 + (i % 3) * .13
		var a = Vector3(-.063, 0, 0)
		var b = Vector3(.063, 0, 0)
		var c = Vector3(-.039, h * .6, .14)
		var d = Vector3(.039, h * .6, .14)
		var tip = Vector3(.08, h, .32)
		for v in [a, b, c, b, d, c, c, d, tip]:
			st.set_uv(Vector2(0, v.y / h))
			st.add_vertex(turn * v)
	st.generate_normals()
	var blade = st.commit()
	var positions = []
	for i in range(700000):
		var x = rng.randf_range(-172, 172)
		var z = rng.randf_range(-106, 170)
		var core = x > -87 and x < 131 and z < 136 and not (x < -18 and z > 53)
		var verge = (
			(z > 53 and z < 59 and x > -17 and x < 79)
			or (z > 40.5 and z < 44.0 and x > -69 and x < 81)
			or (z > 111 and z < 135 and x > 23 and x < 81)
		)
		if core and not verge:
			continue
		var paved = false
		for road_x in map.STREETS_X:
			if absf(x - road_x) < 8.5 and z < 136:
				paved = true
		for road_z in map.STREETS_Z:
			if absf(z - road_z) < 8.5 and x > -121 and x < 130:
				paved = true
		if paved:
			continue
		if absf(map.height_at(x, z) - map.terrain_height(x, z)) > .1:
			continue
		if (absf(x + 104) < 3 or absf(x + 38) < 3) and z > 44 and z < 95:
			continue
		if absf(x + 147 - sin(z * .018) * 9) < 2.5 or absf(x - 151 - sin(z * .018) * 9) < 2.5:
			continue
		if Vector2(x + 110, z - 99).length() < 12 or Vector2(x + 56, z - 62).length() < 9:
			continue
		if _on_path(x, z, 2):
			continue
		var transform = Transform3D(
			Basis(Vector3.UP, rng.randf() * TAU).scaled(Vector3.ONE * rng.randf_range(.55, 1.3)), map.point(x, z)
		)
		if not map.road_end_contains(x, z, .45):
			positions.append(transform)
	var material := ShaderMaterial.new()
	material.shader = load("res://shaders/grass.gdshader")
	material.set_shader_parameter("grass_base", Vector3(.18, .29, .12))
	material.set_shader_parameter("grass_mid", Vector3(.32, .43, .18))
	material.set_shader_parameter("grass_tip", Vector3(.54, .55, .29))
	var batch_count := CozyMeshBatches.spatial(
		g.root, blade, material, positions, "Wind grass", 32, 110, 12, GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
	)
	print(
		"Harbor Hills vegetation: ",
		tree_count,
		" cypress, ",
		positions.size(),
		" grass clumps in ",
...
func _on_path(x: float, z: float, width: float) -> bool:
	...
	for route in [
		...
		var line = b - a
		if p.distance_to(a + line * clampf((p - a).dot(line) / line.length_squared(), 0, 1)) < width:
			return true
	return false
		...
		for j in range(9):
			...
			if clear:
				g.add(
					...
					at,
					Vector3(1.6, 1.8, 1.6),
					["628641", "487740", "789448", "3c7148"][j % 4],
					Vector3(0, a, 0)
				)
