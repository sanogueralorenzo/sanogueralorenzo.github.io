...
func material(color: String, kind: String="plaster") -> Material:
	...
	if kind=="foliage":
		...
		if leaf_texture==null:leaf_texture=preload("res://maps/harbor_hills/textures.gd").leaf_spray()
	...
	m.set_shader_parameter("pattern",1 if kind=="siding" else 2 if kind=="brick" else 3 if kind=="roof" else 4 if kind=="asphalt" else 5 if kind=="paving" else 0)
	materials[key]=m
	return m
...
func shape(kind: String) -> Mesh:
	...
	if kind=="box":mesh=BoxMesh.new()
		...
		for i in range(28):
			...
			var basis=Basis(Quaternion(Vector3.FORWARD,normal));var radius=random.randf_range(.22,.35)
			var shade=random.randf_range(.72,1.22);st.set_color(Color(shade,shade,shade))
			var corners=[Vector2(-1,-1),Vector2(1,-1),Vector2(1,1),Vector2(-1,1)]
			for j in [0,1,2,0,2,3]:
				st.set_uv(corners[j]*.5+Vector2.ONE*.5);st.set_normal((center*Vector3(1,2,1)+normal*.18+Vector3.UP*.2).normalized());st.add_vertex(center+basis*Vector3(corners[j].x*radius,corners[j].y*radius,0))
		mesh=st.commit()
	else:
...
func ribbon(points: Array,width: float,color: String,solid: bool=false,drape: bool=true,finish: String="plaster") -> void:
	var st=SurfaceTool.new();st.begin(Mesh.PRIMITIVE_TRIANGLES)
	...
	for y in range(6):
		for x in range(6):
			for corner in [Vector2(0,0),Vector2(1,0),Vector2(1,1),Vector2(0,0),Vector2(1,1),Vector2(0,1)]:
				...
				st.set_uv(uv);st.set_normal(Vector3.FORWARD);st.add_vertex(Vector3((uv.x-.5)*size.x,-uv.y*size.y,0))
	var node=MeshInstance3D.new();node.mesh=st.commit();node.position=at;node.rotation.y=yaw
	var mat=ShaderMaterial.new();mat.shader=load("res://maps/harbor_hills/cloth.gdshader");mat.set_shader_parameter("tint",Color(color));node.material_override=mat;root.add_child(node)
