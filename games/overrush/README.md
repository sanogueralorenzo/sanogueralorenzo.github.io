...
- Forests use batched tapered trunks and original four-tier faceted conifer crowns with one explicit obstacle body per chunk. One complete crown mesh replaces three overlapping sphere instances per tree, while one consolidated concave collision mesh replaces hundreds of per-tree scene nodes. Deterministic centers, fatal direct impacts, glades, and tested trunk spacing remain unchanged.
