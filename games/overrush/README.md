...
- Each run creates a deterministic, unbounded radial landscape with a softened central summit, broad landforms, folded ridges, and smaller dunes.
...
- Forests use batched low-poly trunks and canopies with one explicit obstacle body per chunk. Trees stay on grass, maintain tested passage spacing, and never qualify as rideable landings or refresh the air boost.
...
- Cool atmospheric depth, warm sand, readable grass, lit stone landmarks, directional slope shading, SSAO, and rounded spray grains keep slopes and motion readable without changing the Forward+ renderer configuration.
...
- `landscape_layout.gd` owns deterministic biome blending, glades, tree cells, and sparse separated ruin sites.
...
The scope gate locks the project to one entry scene, eleven justified runtime scripts, one shader, eleven inputs, and five movement cues so legacy survivor complexity cannot quietly return. The feature gate checks deterministic seed variation, balanced feature-family distribution, smooth cell boundaries, bounded cache residency, meaningful relief, and a maximum authored-feature grade. The landscape-layout gate verifies that every sampled heading crosses clear dune and forest regions, biome changes remain gradual, tree placement leaves the summit and glades open, and ruins remain sparse and separated. The presentation gate checks biome-aware spray, warm/cool separation, sightline-safe atmospheric depth, a non-placeholder rider silhouette, rounded particles, close framing, continuous speed FOV, and the reduced-motion override. The terrain sweep samples 16 radial lines over 12 km and requires every heading's reachable 512 m-wide line fan to encounter bowls, ridges, kickers, split lines, and multiple rock passages while rejecting insufficient net descent, non-finite terrain, repetitive cone geometry, or abrupt local rises. The streaming gates compare adjacent mesh borders, normals, and biome weights; verify collision continuity plus obstacle identity; enforce grass-only tree placement, open passages, and non-rideable landmarks; simulate 22 km of repeated rebased travel; and check bounded chunk residency. These automated gates establish a foundation; they do not substitute for long-session streaming soaks or hands-on movement playtests.
