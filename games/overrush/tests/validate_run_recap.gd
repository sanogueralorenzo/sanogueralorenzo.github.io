...
func _run() -> void:
	...
	var copy_report: Button = scene.get_node("HUD/GameOverOverlay/CopyReport")
	...
	_expect(copy_report.visible and copy_report.text == "COPY RUN REPORT", "The recap should expose a one-action playtest handoff beside retry.")
	...
	copy_report.pressed.emit()
	var copied_report = JSON.parse_string(scene._profile.get_latest_playtest_report_json())
	var copied_run: Dictionary = copied_report.get("run", {}) if copied_report is Dictionary else {}
