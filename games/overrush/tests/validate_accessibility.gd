...
func _validate_onboarding_model() -> void:
	...
	onboarding.reset()
	_expect("WASD + MOUSE" in onboarding.get_message() and "any direction" in onboarding.get_message(), "Guidance should begin with independent movement and camera control.")
	_expect("LEFT + RIGHT STICKS" in onboarding.get_message(true), "Controller guidance should teach movement and camera control with both sticks.")
	onboarding.update(0.1, true, false, false)
	_expect(onboarding.step == RunOnboardingModel.DASH, "Directional movement should advance guidance to the dash concept.")
	...
	_expect(onboarding.step == RunOnboardingModel.FLOW, "Performing movement concepts should advance into movement-skill scoring instead of ending guidance.")
	_expect("52 M/S" in onboarding.get_message().to_upper() and "MOMENTUM" in onboarding.get_message().to_upper(), "Guidance should explain how Flow starts and that it rewards execution rather than combat power.")
	onboarding.update(RunOnboardingModel.INFORMATION_ADVANCE_SECONDS, false, false, false)
	_expect(onboarding.step == RunOnboardingModel.THREATS, "Flow guidance should advance into attack-zone literacy after its bounded display window.")
