...
func _init() -> void:
	...
	boost.leave_surface()
	_expect(boost.try_use(), "Leaving valid sand should expose exactly one air boost.")
	...
	boost.leave_surface()
	_expect(not boost.try_use(), "Leaving a non-sand contact must not create a new charge.")
	_expect(boost.land(true) and boost.available, "A valid sand landing should refresh the spent boost.")
	boost.leave_surface()
	_expect(boost.try_use() and not boost.try_use(), "Every later airtime should still contain exactly one charge.")

	if _failures.is_empty():
		print("Air boost state validation passed — one charge, sand-only refresh, and non-sand rejection agree.")
		quit(0)
	else:
