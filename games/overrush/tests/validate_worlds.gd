...
func _run_validation() -> void:
	...
	for index in range(SEED_COUNT):
		...
		if seed_errors.is_empty():
			print(
				"PASS seed %d — samples %d, alternate %s, slope %.3f, sightline turn %.1f°"
				% [
					seed,
	...
	if failures.is_empty():
		print(
			"VALIDATED %d SEEDS — %d unique layouts, %d alternates, max slope %.3f, max 200–400 m turn %.1f°"
			% [SEED_COUNT, fingerprints.size(), alternate_count, max_route_slope, max_sightline_turn]
		)
		quit(0)
	else:
