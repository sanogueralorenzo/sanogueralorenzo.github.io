...
func _run() -> void:
	...
	_expect(
		world._ruin_block_mesh is ArrayMesh
			...
			and ProceduralDesert.RUIN_VISUAL_SEGMENTS >= 52
			and ProceduralDesert.RUIN_COLLISION_VOLUMES == 9,
		"Ruin landmarks should use a complete temple arch, relief band, cap, base, pylon, and stepped pediment silhouette rather than five scaled boxes.",
	)
	_expect(
