...
public partial class NativeStage3D : Node3D
{
    ...
    ShaderMaterial sky = null!;
    MeshInstance3D backdrop = null!;
    ...
    public override void _Ready()
    {
        ...
        {
            BackgroundMode = Godot.Environment.BGMode.Color, BackgroundColor = new("16394c"),
        ...
        AddChild(Lens);
        sky = new ShaderMaterial { Shader = GD.Load<Shader>("res://source/presentation/sky-background.gdshader") };
        backdrop = new MeshInstance3D { Mesh = new PlaneMesh { Size = new(160, 160) }, MaterialOverride = sky, CastShadow = GeometryInstance3D.ShadowCastingSetting.Off };
        AddChild(backdrop); Follow(Vector2.Zero);
    }
    public void Follow(Vector2 center, float sizeOverride = 0)
    {
        ...
        Lens.LookAt(target); sky.SetShaderParameter("camera_offset", new Vector2(center.X, center.Y) * .001f); backdrop.Position = target + Vector3.Down * 22;
    }
    public void Advance(float clock) { sky.SetShaderParameter("clock", clock); DioramaSurface.Advance(clock); }
