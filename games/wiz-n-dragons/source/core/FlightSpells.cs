...
public sealed partial class Flight
{
    ...
    public int RunesDropped { get; private set; }
    public int RunesExploded { get; private set; }
    public int FireballRicochets { get; private set; }
    public int TetherPulls { get; private set; }

    void UpdateSpells(float dt)
    {
        var forward = new Vector2(MathF.Sin(Heading), -MathF.Cos(Heading));
        for (int w = 0; w < Spells.Length; w++)
        {
            Cooldowns[w] = Math.Max(0, Cooldowns[w] - dt * FireRateMultiplier);
            int rank = Spells[w];
            if (rank == 0 || Cooldowns[w] > 0 || Shots.Count >= 360) continue;
            float range = w == 4 ? WardRadius : w == 3 ? 285 * Area : 570;
            ...
            if (w == 2)
            {
                if (Velocity.Length() < Speed * .2f || Shots.Count(s => !s.Hostile && s.Kind == SpellKind.Rune && s.Life > 0) >= 8) continue;
                var behind = Position - forward * 70;
                Shots.Add(new() { Kind = SpellKind.Rune, Position = behind, Previous = behind, Damage = 38, Life = 10, Radius = (109+(rank-1)*30)*Area });
                RunesDropped++; Events.Add(new("runeDrop",behind));
            }
            else
            {
                ...
                if (w == 5)
                {
                    var aim = SkyWorld.Unit(target.Position-Position);
                    for (int orb = 0; orb < rank; orb++)
                    {
                        ...
                        Shots.Add(new() { Kind=SpellKind.Arcane, Position=start, Previous=start, Velocity=dir*400, Damage=damage, Life=2.4f, Radius=9 });
                    }
                    ArcaneCasts++;
                }
                else if (w == 4)
                ...
                else
                {
                    ...
                    var dir = SkyWorld.Unit(target.Position-mount);
                    int count = w == 0 ? rank : 1;
                    for (int i=0;i<count;i++)
                    {
                        var start = mount + dir*24 + new Vector2(-dir.Y,dir.X)*((i-(count-1)/2f)*9);
                        Shots.Add(new() { Kind=(SpellKind)w, Position=start, Previous=start, Velocity=dir*(w==1?580:650), Damage=damage, Life=w==0?2.4f:1.15f, Radius=6, Pierce=w==1?1:0, Bounces=w==0?1:0 });
                    }
                    Events.Add(new(w==0?"shot":"tether",mount));
                }
            }
    ...
    void UpdateShots(float dt)
    {
        foreach (var s in Shots)
        {
            ...
            if (s.Kind == SpellKind.Arcane && !s.Hostile)
            {
                ...
                if (target != null) s.Velocity = SkyWorld.Unit(Vector2.Lerp(s.Velocity/400,SkyWorld.Unit(target.Position-s.Position),1-MathF.Exp(-dt*7)))*400;
            }
            s.Previous=s.Position; s.Position+=s.Velocity*dt; s.Life-=dt; s.Age+=dt;
            if (s.Hostile)
            {
                // Boss bombs pass over wizards and terrain, then burst at their fixed landing point.
                if (s.Life <= 0)
                {
                    s.Position = s.Target;
                    BossExplosions++;
            ...
            foreach(var e in Enemies)
            {
                ...
                if(s.Kind==SpellKind.Tether && e.Health>0)
                {
                    e.Pull=.4f; TetherPulls++; Events.Add(new("pull",Position,Spells[1],e.Position));
                }
                if(s.Kind==SpellKind.Fireball && s.Bounces>0)
                {
                    ...
                    s.Velocity=SkyWorld.Unit(next.Position-e.Position)*650;
                    s.Bounces--; FireballRicochets++; Events.Add(new("ricochet",s.Position)); break;
                }
