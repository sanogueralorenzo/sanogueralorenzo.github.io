...
public partial class ArtSample : Node3D
{
    ...
    Node3D rocky = null!;
    int islandStyle;
    static readonly uint[] IslandStyles = [921, 3888717872, 582933664];
    ...
    void ShowIsland()
    {
        ...
        rocky = EnvironmentArt3D.Build(new Place("sample-rock", PlaceKind.Island, default, islandStyle == 0 ? 155 : 260, IslandStyles[islandStyle]));
    ...
    public override async void _UnhandledKeyInput(InputEvent ev)
    {
        ...
        if (key.Keycode == Key.F12)
        {
            ...
            var file = folder + "/" + DateTime.Now.ToString("yyyyMMdd-HHmmss-fff") + "-art-" + (close ? "detail" : "scale") + ".png";
