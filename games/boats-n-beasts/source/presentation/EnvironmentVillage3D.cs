...
public static partial class EnvironmentArt3D
{
    private static void Lighthouse(Sculptor art)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            var v0 = P(b, 1.84f, .295f); var v1 = P(a, 1.84f, .295f); var v2 = P(a, 2.13f, .295f); var v3 = P(b, 2.13f, .295f);
            ...
            art.Tube(P(a, 1.79f, .32f), P(a, 2.17f, .32f), .023f, .023f, iron, 5);
        }
        art.Tube(new(0, 2.13f, 0), new(0, 2.18f, 0), .35f, .35f, roof.Darkened(.22f), 8);
        art.Tube(new(0, 2.18f, 0), new(0, 2.62f, 0), .43f, .02f, roof, 8);
        art.Ellipsoid(new(0, 2.64f, 0), new(.065f, .095f, .065f), roof.Lightened(.12f), 6, 4);
        for (int i = 0; i < 5; i++)
        {
    ...
    private static void MarketStall(Sculptor art)
    {
        ...
        {
            art.RoundedBox(new(x * .64f, .71f, z * .37f), new(.075f, 1.42f, .075f), .009f, wood);
            art.RoundedBox(new(x * .64f, .07f, z * .37f), new(.10f, .14f, .10f), .012f, wood.Darkened(.15f));
        }
        for (int side = -1; side <= 1; side += 2)
        {
            art.Tube(new(-.72f, 1.41f, side * .41f), new(.72f, 1.41f, side * .41f), .036f, .036f, edge, 5);
            art.Tube(new(side * .64f, 1.43f, -.43f), new(side * .64f, 1.43f, .43f), .028f, .028f, wood, 5);
        }
        // Stitched stripes follow the same bowed canopy; each has a rounded hanging hem.
        for (int stripe = 0; stripe < 8; stripe++)
        {
            float x = -.68f + stripe * .17f;
            Color cloth = stripe % 2 == 0 ? teal : canvas;
            Vector3 P(float xx, float t) => new(xx, 1.42f + .15f * Mathf.Sin(t * Mathf.Pi), -.47f + t * 1.0f);
            for (int j = 0; j < 6; j++)
            {
                float a = j / 6f, b = (j + 1) / 6f;
                art.Quad(P(x, b), P(x + .17f, b), P(x + .17f, a), P(x, a), cloth);
            }
            for (int j = 0; j < 4; j++)
            {
                float a = j / 4f, b = (j + 1) / 4f;
                Vector3 Hem(float t) => new(x + t * .17f, 1.28f - .04f * Mathf.Sin(t * Mathf.Pi), .535f);
                art.Quad(new(x + a * .17f, 1.42f, .53f), Hem(a), Hem(b), new(x + b * .17f, 1.42f, .53f), cloth.Darkened(.05f));
            }
        }
        for (int i = 0; i < 7; i++)
        ...
        for (int side = -1; side <= 1; side += 2)
        {
            art.RoundedBox(new(side * .40f, .64f, .43f), new(.37f, .08f, .30f), .009f, wood);
            for (int i = 0; i < 6; i++)
                art.Ellipsoid(new(side * .40f + (i % 3 - 1) * .09f, .73f, .345f + i / 3 * .10f), new(.057f, .056f, .053f), side < 0 ? new("c39545") : new("86a54c"), 7, 4);
        }
        for (int i = 0; i < 3; i++)
        {
            float x = -.13f + i * .13f;
    ...
    private static void Windmill(Sculptor art)
    {
        ...
        art.Tube(new(0, .10f, 0), new(0, 1.43f, 0), .41f, .27f, plaster, 8);
        art.Tube(new(0, 1.40f, 0), new(0, 1.91f, 0), .43f, .018f, new("c87d39"), 8);
        art.Tube(new(0, 1.40f, 0), new(0, 1.45f, 0), .43f, .42f, wood, 8);
        ...
        art.Tube(new(0, 1.44f, .21f), new(0, 1.44f, .58f), .09f, .09f, wood, 8);
        for (int blade = 0; blade < 4; blade++)
        {
            art.Transform = mill * new Transform3D(Basis.FromEuler(new(0, 0, Mathf.Pi / 4 + blade * Mathf.Pi / 2)), new(0, 1.44f, .59f));
            art.RoundedBox(new(0, .68f, 0), new(.047f, 1.26f, .045f), .006f, wood);
            art.RoundedBox(new(.27f, .80f, 0), new(.04f, .96f, .045f), .006f, wood);
            for (int panel = 0; panel < 3; panel++)
            {
                float y = .34f + panel * .31f;
                art.Quad(new(.025f, y, .02f), new(.25f, y, .02f), new(.25f, y + .30f, .04f), new(.025f, y + .30f, .04f), cloth.Darkened(panel % 2 * .035f));
                art.RoundedBox(new(.135f, y, .04f), new(.34f, .026f, .035f), .004f, cut);
            }
            art.RoundedBox(new(.135f, 1.27f, .025f), new(.34f, .04f, .055f), .005f, cut);
        }
        art.Transform = mill;
        art.Tube(new(0, 1.44f, .60f), new(0, 1.44f, .69f), .14f, .12f, new("4e5450"), 10);
        art.Ellipsoid(new(0, 1.44f, .70f), new(.055f, .055f, .025f), cut, 8, 4);
