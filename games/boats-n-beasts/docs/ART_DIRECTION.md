...
Simulation `(x,y)` maps to native `(x/100,0,y/100)`; bows point along -Z. Preserve orthographic scale 0.74 pixels per simulation unit and 0.78 ground-plane foreshortening (about 51° above the water). HUD projection and sailing clicks use the same camera.
