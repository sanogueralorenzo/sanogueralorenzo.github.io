...
Shallows now bake a dedicated procedural shader beneath the island art, using the same seeded beach contour with noisy depth transitions. The ocean uses warped depth fields and quiet directional ripples. The rock has explicit shoulder ledges. Native comparison still shows a more graphic result than the concept; continue working on natural shore structure, building proportions, foliage and creature volume. Latest user-directed gameplay exception: level-up choices now appear immediately at sea rather than waiting for a harbor.
