...
Reference visual pass: see ART_DIRECTION.md for requirements. All changes are presentation-only. SceneryCache now draws behind OceanView with premultiplied-alpha blending, preventing dark translucent shelves. Surf uses the same seeded IslandRing as the island mesh and runs outside the static cache. Creature shells use nested faceted rings; serpent atlas cells are wider. Bounded orb/creature trail histories clear on Reset and remove dead entries. Mage crystal uses a screen-upright extrusion independent of hull heading. Fish shadows must draw within their existing transform without calling the transform-resetting DrawEllipse helper. Debug/Release and live checks recorded in QUALITY.md.
