...
Completed the [sparse rewards goal](OCEAN_REWARDS_GOAL.md). Offshore islands have a 15% chest roll, then up to 32 bounded shoreline attempts; invalid sites are omitted. Chests sit 22 units inside a shared coast edge, with a clear boat approach and an art reservation that suppresses overlapping props. They are stationary, have gold framing and a small glint, and award 35–55 gold from within 140 units. The former open-water and guaranteed home chests are removed.

Barrels have a 20% candidate chance per offshore chunk, at most one per chunk, 900-unit minimum spacing and further shore/encounter clearance. All acceptance depends on seed/coordinates, including unloaded neighbor candidates. Two chunks apart cannot conflict with the barrel spacing bound, so the eight-neighbor check suffices. Small native meshes have bulged wooden staves, dark hoops, varied orientation and mild bobbing. Boat-hull contact breaks them for 3–6 gold, with short wood fragments and a splash. Neither pickup creates a large notification panel; a local gold number lasts 1.2 seconds. Barrels have no chart icon or overhead marker. Collected chests leave the chart.

Debug and Release pass with zero warnings/errors. Both import/gameplay sessions are clean. The initial chest's gray bands looked too similar to the barrel, so the final presentation adds gold bands and a rim. That refinement changes only chest colors and its decorative rim; placement, collection and balance behavior stay the same.

### Native verification

- **Exploration, seed `3235500626`:** normal ×1 Cutter play reaches a 571.19-radius crescent more than ten leagues east and collects its chest for 43 gold from the water. Two barrels about 2,087 units apart each give 4 gold. The captured 25-chunk regions hold 0–4 barrels, including depleted ones, with broad empty stretches in the actual view. The second barrel chunk unloads during eastern exploration and is reloaded/depleted on return to `(4360.432,-311.594)`. The voyage reaches 7:17.89 combat time, 130 kills, one chest and two barrels, with full hull. This session predates the gold-trim refinement only.
- **Final build, seed `2242038093`:** a 94.01-radius headland has a clearly visible gold-trimmed chest beside a retained palm. Ordinary sailing collects 53 gold with the boat in water, then breaks an isolated barrel for 3 gold. Sailing east to `(665.444,-2131.982)` unloads the chest's chunk; returning to `(-2070.244,-1303.568)` restores the same island and chest coordinates/style, with `depleted=True`, no chest mesh/chart icon and no second reward. The final capture has 100/100 hull, 66 kills, exactly one chest and one barrel collected, and 25 active chunks. It reaches 4:53.76 combat time over 253.1 seconds of active wall time; the speed control briefly passed through ×2/×3 early, then returned to ×1 for the pickup/revisit checks.

At the final revisit, the encounter/scenery caches hold 6/4 entries; mean/p95/p99 frame times are 12.05/16.67/18.75 ms, simulation 0.080 ms and presentation submission 0.330 ms on M3 Max, Metal Forward+. The earlier seven-minute route ends at 16.74/16.67/19.22 ms. These are observations across two routes, not worst-case performance or a statistical validation of spawn percentages. Placement chances are configuration values before rejection. Native checks cover small and giant island access, barrel contact, reward amounts, depletion and streaming; no exhaustive seed/family sweep, all-boat contact check or full 22-minute economy replay was performed. Combat pacing code is unchanged. No automated tests, injected gameplay state or imported art. [Unedited captures and logs](../evidence/README.md).
...
Puffers enter gradually over minutes 2–4, serpents over 6–8 and rays over 10–12. Species health, damage and base swimming speeds stay fixed; existing per-enemy 0.85–1.4 speed variation, serpent dashes and other attack behaviors remain. Distance still affects gold and fishing. The pause objective and handbook explain the 22-minute arrival and active-time rule. F12 telemetry now records the population curve, boss deadline and actual per-enemy swimming speed.

Debug and Release pass with zero warnings/errors. Final import and gameplay logs are clean. Two existing core script UID files, regenerated by the first import, are retained so subsequent imports no longer warn about missing metadata. Source review confirms combat health, movement and damage no longer reference the distance tier; the tier remains in rewards/fishing.

### Sustained native play

Final Gunboat/Cutter voyage seed `3937392081` reaches **victory at 22:11** (combat clock 1331.9384). It uses normal UI controls, earned Cannon/Lightning, Reload, Reach and Speed upgrades, with no repairs, injected state or automated tests. Most of the long check uses the game's ×3 control; it returns to ×1 at 21:22. Measured unpaused wall time is 478.1 seconds, distinct from the 22-minute game clock. The boat sails roughly four leagues offshore early, then holds position through the buildup.

- At 3:16, the boat is at `(2541.7878,3146.1145)`, beyond the former three-league trigger, with full hull, ordinary crab/puffer stats and no boss.
- Pause and handbook captures 17 seconds apart have identical combat clock `214.87617`, director state, enemy positions and speeds. Menu time does not advance difficulty or boss arrival.
- The observed peak population grows from 2 early to 6 around ten minutes, 11 around fifteen, 17 at eighteen and 18 before the boss. These are actual populations for a strong damage build; the configured ceilings are higher. All four regular species appear. Hull remains 100 through 21:58.
- At `1318.9636` seconds (21:58.96), the boss is absent and no bombs have been thrown. At `1330.6497`, Crownclaw is present with age 10.674545 seconds, consistent with the 22:00 threshold, and eight regular escorts. Three boss blast hits account for the final hull of 22/100. Victory clears enemies and shots normally.

All captures retain 25 loaded chunks. At victory, the scenery cache holds seven places and mean/p95/p99 frame times are **9.04/12.96/15.43 ms**, with simulation **0.083 ms** and submission **0.538 ms** on M3 Max, Metal Forward+. Earlier samples vary, including roughly 20 ms means at the start; these measurements do not establish worst-case performance. This verifies one developed build surviving the intended duration, not balance across every boat/build or guaranteed survival. The 64-enemy ceiling, post-victory plateau and maximum crowd were reviewed in code rather than separately stress-tested. [Unedited captures and complete final log](../evidence/README.md).
...
Debug/Release pass with zero warnings/errors; native runtime log is clean. Gunboat and Mage sailed beyond three leagues into boss encounters with rays pursuing and only boss bombs in hostile-shot telemetry. Paired paused captures 1.817 seconds apart have identical simulation and three in-flight bombs. Resuming produces visible red domes at the recorded targets. Gunboat avoided all three explosions (blast hits remained zero), then died from close-range damage. A stationary Mage pass shows domes overlapping the boat, but contact-hit invulnerability prevented an isolated boss-damage observation. Damage range and amount were reviewed in code; direct bomb damage, the below-half-health volley and Aura interception were not separately verified in native play. Final sampled frame mean/p95/p99 were 8.36/8.33/8.34 ms on M3 Max; this was a focused encounter check, not a sustained balance run. No automated tests or injected state. [Native evidence](../evidence/README.md).
