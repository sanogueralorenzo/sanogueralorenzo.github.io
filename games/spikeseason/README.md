...
![Coastal gameplay](evidence/3d/maps/final-coast-gameplay.png)

## Launch

Use Godot 4 with **Forward+** support. Verified with Godot **4.7.2**, Metal, on macOS / Apple M3 Max.

```sh
godot --path games/spikeseason
```

Or run `./games/spikeseason/launch.sh`, or import `project.godot` in Godot and press **F5**. The project opens directly into the introduction. No other game directory is needed. The source project is the independently runnable delivery; platform-specific export templates are not bundled.
...
- `scripts/three_d/geometry.gd`, `ball_effects.gd`, `shaders/three_d/`: meshes, materials, paint finish, wind, water, and contact feedback.
...
See [runtime verification](VERIFICATION.md) for before/after views, the rally recording, independent reviews, demonstrated results, and limitations. No automated tests or gameplay bots were created. The supplied art reference is not included as an in-game image.
