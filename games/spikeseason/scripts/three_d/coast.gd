...
func batch_static() -> void:
	# Merge inert scenery by shared material. Keep wind-bearing crowns independent.
	...
	for child in get_children():
		if child is MeshInstance3D and not child.has_meta("moving_foliage"):
			...
			remove.append(child)
	for material in groups: G.instance(self,groups[material].commit(),Vector3.ZERO,material)
