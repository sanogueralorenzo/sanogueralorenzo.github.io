...
[examples/ci/jury-npm-publish.yml](examples/ci/jury-npm-publish.yml) shows the full release shape: `package-release-fixtures` runs `npm --prefix "$JURY_PACKAGE_DIR" run fixtures:package-release:check` and `npm --prefix "$JURY_PACKAGE_DIR" run fixtures:package-release:drift` before any publication dry run, uploads `jury-package-release-evidence` with rollback, replacement, and archive drift remediation audit examples, exports `retained-package-release-evidence-manifest.json`, uploads it as `jury-package-release-archive-manifest`, and `package-release-evidence-replay` downloads both artifacts before replaying evidence with `--fixture-dir` and the manifest with `--verify-manifest "$JURY_PACKAGE_RELEASE_MANIFEST_PATH"`. The replay job replays `archive-drift-remediation-audit-handoff.json` against the retained manifest and remediation audit before summary generation, writes a `GITHUB_STEP_SUMMARY` section with the failed package version, failed tarball name, replacement package version, failed archive evidence, replacement archive evidence, and remediation approver, saves the same content as `jury-package-release-replay-summary.md`, writes `jury-package-release-replay-summary-diagnostics.json` to prove the summary lines matched retained failed and replacement archive evidence, records `jury-package-release-replay-summary-diagnostics-retention-handoff.json` for retained diagnostics promotion, replays that generated handoff against the retained manifest, diagnostics JSON, retained archive handoff, remediation approval, and artifact provenance, replays `jury-package-release-replay-summary-expiry-handoff.json` against the retained manifest, remediation audit, reconstructed summary content, and retained evidence artifact provenance, and uploads all three files as `jury-package-release-replay-summary`. `dry-run-publication` has `needs: package-manifest` and `needs: package-release-evidence-replay`, uploads `jury-pack-dry-run.json` and `jury-pack-dry-run-record.json` as `jury-package-dry-run`, and keeps CI artifacts for 90 days with `retention-days: 90`. If the replay job, remediation audit handoff CI workflow enforcement, replay summary, replay summary diagnostics, replay summary diagnostics retention handoff, replay summary diagnostics retention handoff CI replay enforcement, replay summary expiry handoff CI workflow enforcement, replay summary retention, or archive drift check fails, use the package release evidence replay failure, remediation audit handoff CI workflow enforcement failure, replay artifact summary failure, replay summary CI workflow diagnostics failure, replay summary diagnostics retention handoff failure, replay summary diagnostics retention handoff CI replay enforcement failure, replay summary expiry handoff CI workflow enforcement failure, replay summary diagnostics retention handoff schema failure, replay summary retention failure, or retained package release manifest replay failure section in [TROUBLESHOOTING.md](TROUBLESHOOTING.md) before reusing or replacing the uploaded audit artifact. `publish` downloads and verifies the dry-run artifact before the step that maps `secrets.NPM_TOKEN` to `NODE_AUTH_TOKEN`. The workflow requires `dry_run_reviewer`; the verification step writes `packageVersion`, `tarballName`, and `reviewedBy` to `GITHUB_STEP_SUMMARY` so the release page records the package identity and who reviewed it before credentials were exposed. Keep `NODE_AUTH_TOKEN` in `secrets.NPM_TOKEN`.
...
For a failed publication, retain `jury-package-dry-run`, `jury-package-release-evidence`, `jury-package-release-replay-summary`, `jury-package-release-replay-summary.md`, `jury-package-release-replay-summary-diagnostics.json`, `jury-package-release-replay-summary-diagnostics-retention-handoff.json`, `jury-package-release-replay-summary-expiry-handoff.json`, `jury-pack-dry-run-record.json`, `failed-npm-view.json`, `downstream-failure-gate.json`, `rollback-audit.json`, `archive-drift-remediation-audit.json`, `archive-drift-remediation-audit-handoff.json`, and the `GITHUB_STEP_SUMMARY`. For the replacement patch, retain those failed-version artifacts plus `replacement-npm-view.json`, `replacement-downstream-gate.json`, `replacement-patch-audit.json`, and any failed-version deprecation result.

Validate `jury-package-release-replay-summary-diagnostics.json` against [schemas/package-release-replay-summary-diagnostics.schema.json](schemas/package-release-replay-summary-diagnostics.schema.json) before closing the release archive. The diagnostics record must match the retained manifest, `archive-drift-remediation-audit.json`, and saved summary file so failed and replacement release archive evidence cannot drift from the CI workflow summary. Store `jury-package-release-replay-summary-diagnostics-retention-handoff.json` with it and validate that handoff against [schemas/package-release-replay-summary-diagnostics-retention-handoff.schema.json](schemas/package-release-replay-summary-diagnostics-retention-handoff.schema.json) so the archive records the source artifact, source job, retention window, workflow run, source revision, and reviewer for diagnostics promotion. Store `archive-drift-remediation-audit-handoff.json` beside the remediation audit and validate it against [schemas/package-release-remediation-audit-handoff.schema.json](schemas/package-release-remediation-audit-handoff.schema.json) so the failed and replacement archives retain the drift evidence, restored evidence, verification commands, regenerated manifest, diff review, workflow run, source revision, approver, and approval time.
