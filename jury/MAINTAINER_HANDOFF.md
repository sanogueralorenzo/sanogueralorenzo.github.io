...
- [examples/ci/jury-code-change-adoption.yml](examples/ci/jury-code-change-adoption.yml): reusable workflow that publishes signed retry and accept code-change adoption bundles for downstream verification.
...
- [examples/ci/fixtures/code-change-adoption-key-policy](examples/ci/fixtures/code-change-adoption-key-policy): signed retry and accept code-change adoption bundles plus a trusted key policy manifest for downstream verification.
...
- [schemas/package-release-remediation-audit-handoff.schema.json](schemas/package-release-remediation-audit-handoff.schema.json): JSON schema contract for retained archive drift remediation audit handoff records.
...
The test suite covers the CI adoption guide, code-change adoption fixture, signed code-change adoption producer workflow, key-policy-backed signed code-change adoption downstream fixtures, package publication notes, dry-run release publication checklist guidance, dry-run publication artifact handoff, dry-run artifact retention expectations, package release evidence artifact upload guidance, package release evidence artifact download and replay guidance, package release evidence replay failure troubleshooting for package rollback and replacement audits, package release evidence retention policy for failed and replacement release artifacts, package release artifact provenance checks for retained failed and replacement evidence, retained package release evidence manifest export, schema validation, verification, archive drift checking, archive drift remediation audit records, archive drift remediation audit handoff records, archive drift remediation audit handoff schema failure troubleshooting, archive drift remediation audit handoff CI workflow enforcement, archive drift remediation audit handoff CI workflow enforcement failure troubleshooting, CI handoff, replay summary retention handoff, replay summary diagnostics retention handoff, replay summary diagnostics retention handoff CI replay enforcement, replay summary diagnostics retention handoff CI replay enforcement failure troubleshooting, replay summary expiry handoff CI workflow enforcement, replay summary expiry handoff CI workflow enforcement failure troubleshooting, replay summary diagnostics retention handoff schema failure troubleshooting, replay summary retention failure troubleshooting, and replay troubleshooting for failed and replacement release archives, post-publication package metadata comparison guidance, downstream verification rollback notes, replacement patch supersedence evidence, package release evidence fixture examples, package release evidence schema validation, package release fixture workflow gating, dry-run publication summary output, dry-run package summary reviewer audit notes, stale dry-run artifact troubleshooting, npm token and provenance release checklist guidance, release metadata, package tarball manifest checks, quickstart, unsigned and signed GitHub Actions producer workflow commands, signed artifact download verification, downstream trusted-producer verification workflow, fixture synchronization, package manifest troubleshooting, troubleshooting failure examples, release checklist links, and this handoff note's references.
...
The reusable [examples/ci/jury-code-change-adoption.yml](examples/ci/jury-code-change-adoption.yml) workflow runs the code-change adoption fixture, signs retry and accept bundles with `secrets.JURY_CI_PRIVATE_KEY`, verifies both unsigned and signed bundles with `bundle preflight`, runs a strict state check, and uploads `verdict.retry.json`, `gate.retry.json`, `review-bundle.retry.json`, `review-bundle.retry.signed.json`, `verdict.accept.json`, `gate.accept.json`, `review-bundle.accept.json`, `review-bundle.accept.signed.json`, and the fixture state JSONL files under the `jury-code-change-adoption` artifact for downstream verification. Downstream consumers can download that artifact, verify `review-bundle.accept.signed.json` with `bundle preflight --key-policy jury-key-policy.json`, import it into `.jury-code-change-downstream`, gate `downstream-verdict.accept.json`, and run a strict check before trusting the accepted verdict. The [examples/ci/fixtures/code-change-adoption-key-policy](examples/ci/fixtures/code-change-adoption-key-policy) fixture includes signed retry and accept bundles, `ci-code-change-public.pem`, a trusted `jury-key-policy.json`, and an untrusted-producer troubleshooting policy so consumers do not need raw public-key flags in every workflow.
...
## Remediation Audit Handoff

The package release fixture directory includes [examples/ci/fixtures/package-release/archive-drift-remediation-audit-handoff.json](examples/ci/fixtures/package-release/archive-drift-remediation-audit-handoff.json) with `jury.package_release_remediation_audit_handoff.v1`. It records the retained manifest, companion failed and replacement release audits, drifted evidence, restored evidence, verification commands, regenerated manifest, diff review, workflow run id, source revision, approving maintainer, and approval time. The fixture validator loads [schemas/package-release-remediation-audit-handoff.schema.json](schemas/package-release-remediation-audit-handoff.schema.json) and checks the handoff against `archive-drift-remediation-audit.json`, retained artifact provenance, failed package identity, and replacement package identity before failed or replacement release archives close.
...
## Remediation Audit Handoff Schema Failure Troubleshooting

Remediation audit handoff schema failure troubleshooting now covers validator failures for `archive-drift-remediation-audit-handoff.json`. It documents local `fixtures:package-release:check` replay, schema-critical field inspection, retained companion file checks, retained manifest relationship checks for failed package version, failed tarball name, replacement package version, workflow run id, source revision, drift evidence, restored evidence, verification commands, regenerated manifest, diff review, and maintainer approval checks against `archive-drift-remediation-audit.json` before a failed or replacement release archive closes.

## Remediation Audit Handoff CI Workflow Enforcement

The publication workflow now runs `Replay Jury package release remediation audit handoff` after retained manifest replay and before replay summary generation. The step rereads `archive-drift-remediation-audit-handoff.json`, the retained manifest, and `archive-drift-remediation-audit.json`; it fails before `dry-run-publication` if failed package identity, replacement package identity, retained companion files, drift evidence, restored evidence, verification commands, run id, source revision, reviewer, or approval time no longer match the failed and replacement release archive evidence.

## Remediation Audit Handoff CI Workflow Enforcement Failure Troubleshooting

Remediation audit handoff CI workflow enforcement failure troubleshooting now covers missing or misordered `Replay Jury package release remediation audit handoff` workflow steps and local replay failures. It documents workflow ordering checks that prove replay runs after manifest replay but before summary generation and `dry-run-publication`, plus a local replay command that compares `archive-drift-remediation-audit-handoff.json` with retained failed and replacement archive evidence.
...
## Replay Summary Expiry Handoff CI Workflow Enforcement

The publication workflow now runs `Replay Jury package release replay summary expiry handoff` after diagnostics retention handoff replay and before uploading `jury-package-release-replay-summary`. The step rereads `jury-package-release-replay-summary-expiry-handoff.json`, the retained manifest, `archive-drift-remediation-audit.json`, the reconstructed replay summary, and retained evidence artifact provenance; it fails before `dry-run-publication` if package identity, reconstructed inputs, summary content, reviewer, 90-day expiry, or evidence artifact retention no longer match the failed and replacement release archive evidence.

## Replay Summary Expiry Handoff CI Workflow Enforcement Failure Troubleshooting

Replay summary expiry handoff CI workflow enforcement failure troubleshooting now covers missing or misordered `Replay Jury package release replay summary expiry handoff` workflow steps and local replay failures. It documents workflow ordering checks that prove replay runs after diagnostics retention handoff replay but before replay summary upload and `dry-run-publication`, plus a local replay command that compares `jury-package-release-replay-summary-expiry-handoff.json` with retained failed and replacement archive evidence.
...
Add a copyable producer-and-consumer code-change adoption handoff workflow that downloads the signed retry and accept artifact, verifies both bundles through the key policy, and gates the accepted verdict in one CI path.
