...
[jury-npm-publish.yml](jury-npm-publish.yml) shows a release workflow where `package-release-fixtures` runs `npm --prefix "$JURY_PACKAGE_DIR" run fixtures:package-release:check` and `npm --prefix "$JURY_PACKAGE_DIR" run fixtures:package-release:drift` before any publication dry run, uploads `jury-package-release-evidence` with rollback, replacement, and archive drift remediation audit examples, exports `retained-package-release-evidence-manifest.json`, and uploads it separately as `jury-package-release-archive-manifest`. `package-release-evidence-replay` downloads both artifacts, replays the evidence with `--fixture-dir`, verifies the retained archive handoff manifest with `--verify-manifest "$JURY_PACKAGE_RELEASE_MANIFEST_PATH"`, and writes a `GITHUB_STEP_SUMMARY` section for failed and replacement release archive evidence. `dry-run-publication` has `needs: package-manifest` and `needs: package-release-evidence-replay`, uploads `jury-package-dry-run`, keeps package release artifacts with `retention-days: 90`, and `publish` downloads and verifies that artifact before the `NODE_AUTH_TOKEN` publish step runs `npm publish --provenance --access public`. If replay or archive drift checking fails, [../../TROUBLESHOOTING.md](../../TROUBLESHOOTING.md) shows how to inspect `jury-package-release-evidence` files, `JURY_PACKAGE_RELEASE_EVIDENCE_DIR`, and `JURY_PACKAGE_RELEASE_MANIFEST_PATH`. Failed and replacement releases must promote those temporary CI artifacts into the release record or incident archive for at least 180 days after replacement downstream verification passes, including retained artifact provenance for workflow, run id, source revision, source job, retention days, and uploaded files. The workflow requires `dry_run_reviewer`, and the verification step writes the checked `packageVersion`, `tarballName`, and `reviewedBy` to `GITHUB_STEP_SUMMARY`.
