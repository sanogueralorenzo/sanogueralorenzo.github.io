...
[jury-code-change-adoption.yml](jury-code-change-adoption.yml) is a reusable workflow that runs [../code-change-adoption](../code-change-adoption), emits `verdict.retry.json`, `gate.retry.json`, `review-bundle.retry.json`, `review-bundle.retry.signed.json`, `verdict.accept.json`, `gate.accept.json`, `review-bundle.accept.json`, `review-bundle.accept.signed.json`, and uploads the state JSONL files so downstream jobs can verify both the actionable retry and the accepted review bundle. Configure `JURY_CI_PRIVATE_KEY` as a repository or organization secret containing a PEM RSA private key; the `attestation-key-id` input defaults to `ci-code-change-adoption`.
...
[jury-npm-publish.yml](jury-npm-publish.yml) shows a release workflow where `package-release-fixtures` runs `npm --prefix "$JURY_PACKAGE_DIR" run fixtures:package-release:check` and `npm --prefix "$JURY_PACKAGE_DIR" run fixtures:package-release:drift` before any publication dry run, uploads `jury-package-release-evidence` with rollback, replacement, and archive drift remediation audit examples, exports `retained-package-release-evidence-manifest.json`, and uploads it separately as `jury-package-release-archive-manifest`. `package-release-evidence-replay` downloads both artifacts, replays the evidence with `--fixture-dir`, verifies the retained archive handoff manifest with `--verify-manifest "$JURY_PACKAGE_RELEASE_MANIFEST_PATH"`, writes a `GITHUB_STEP_SUMMARY` section for failed and replacement release archive evidence, saves the same content as `jury-package-release-replay-summary.md`, and uploads it as `jury-package-release-replay-summary`. `dry-run-publication` has `needs: package-manifest` and `needs: package-release-evidence-replay`, uploads `jury-package-dry-run`, keeps package release artifacts with `retention-days: 90`, and `publish` downloads and verifies that artifact before the `NODE_AUTH_TOKEN` publish step runs `npm publish --provenance --access public`. If replay or archive drift checking fails, [../../TROUBLESHOOTING.md](../../TROUBLESHOOTING.md) shows how to inspect `jury-package-release-evidence` files, `JURY_PACKAGE_RELEASE_EVIDENCE_DIR`, and `JURY_PACKAGE_RELEASE_MANIFEST_PATH`. Failed and replacement releases must promote those temporary CI artifacts into the release record or incident archive for at least 180 days after replacement downstream verification passes, including the retained replay summary artifact, retained artifact provenance for workflow, run id, source revision, source job, retention days, and uploaded files. The workflow requires `dry_run_reviewer`, and the verification step writes the checked `packageVersion`, `tarballName`, and `reviewedBy` to `GITHUB_STEP_SUMMARY`.
