...
5. Use [examples/ci/jury-code-change-adoption.yml](examples/ci/jury-code-change-adoption.yml) when downstream jobs need retry and accept code-change review bundles from the checked-in adoption fixture.
...
The retry bundle remains available as `review-bundle.retry.json` so consumers can inspect the original `docs/checkout-notes.md` scope objection and confirm the accepted bundle carries the later `ev_scope_corrected` evidence and resolved objection state.
