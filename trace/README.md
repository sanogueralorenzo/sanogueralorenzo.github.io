...
trace session recap <session> --field agents
trace session recap <session> --field lifecycle
...
trace record --check-session --end-session --validation "npm test"
...
node trace/bin/trace.mjs session current --strict
node trace/bin/trace.mjs session show <session>
node trace/bin/trace.mjs session recap <session>
node trace/bin/trace.mjs session recap <session> --field agents
node trace/bin/trace.mjs session recap <session> --field lifecycle
...
node trace/bin/trace.mjs record --check-session --end-session --validation "npm test"
...
`trace init`, `trace enable`, and `trace disable` return one schema-stable JSON result each so setup scripts can call them directly. `trace enable` installs managed `pre-commit`, `prepare-commit-msg`, and `post-commit` git hook blocks. The pre-commit hook blocks raw transcript or checkpoint-shaped files from being committed under `.trace/`. The prepare hook adds `Trace-Checkpoint` and `Trace-Session` trailers to the commit message. The post-commit hook writes a compact memory file under `.trace/commits/`, stores the raw checkpoint payload on the local `refs/trace/checkpoints` git ref, appends a local `trace-record` close note, and clears the current session pointer so the next task starts cleanly.
