...
stop_existing_runtime() {
  if command -v codex-app-server >/dev/null 2>&1; then
    codex-app-server sessions watch thread-titles stop >/dev/null 2>&1 || true
  fi
}
...
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/codex-app-server-target}"
...
BIN_PATH="$TARGET_DIR/release/codex-app-server"
...
TMP_PATH="$DEST_DIR/.tmp-codex-app-server-$$"
cp "$BIN_PATH" "$TMP_PATH"
chmod +x "$TMP_PATH"
mv -f "$TMP_PATH" "$DEST_DIR/codex-app-server"

echo "Installed CLI: $DEST_DIR/codex-app-server"
