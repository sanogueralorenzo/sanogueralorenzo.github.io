...
fn run_wrapper(mode: Mode, options: WrapperOptions) -> u8 {
    ...
    let output_last_message = if let Some(path) = options.output_last_message.clone() {
        ...
        temp_file_path("codex-app-server-last-message", "txt")
    };
...
fn print_noninteractive_command_help() {
    println!("Usage:");
    println!("  codex-app-server noninteractive run|resume|review ...");
    println!();
    println!("Description:");
...
fn print_noninteractive_subcommand_help(mode: &Mode) {
    match mode {
        Mode::Run => {
            println!("Usage:");
            println!(
                "  codex-app-server noninteractive run [wrapper-options] [-- codex-exec-options]"
            );
            println!();
            println!("Description:");
        ...
        Mode::Resume => {
            println!("Usage:");
            println!(
                "  codex-app-server noninteractive resume [wrapper-options] [-- codex-exec-resume-options]"
            );
            println!();
            println!("Description:");
        ...
        Mode::Review => {
            println!("Usage:");
            println!(
                "  codex-app-server noninteractive review [wrapper-options] [-- codex-exec-review-options]"
            );
            println!();
            println!("Description:");
