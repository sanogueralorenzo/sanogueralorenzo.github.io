use crate::shared::models::{DeleteResult, SessionMeta, SessionResultReason, SessionResultStatus};
...
impl Drop for TitleWriteLock {
    fn drop(&mut self) {
        ...
        if result != 0 {
            ...
            eprintln!(
                "[codex-app-server:title-lock] release failed path={} error={}",
        ...
        eprintln!(
            "[codex-app-server:title-lock] released path={}",
            self.lock_path.display()
        );
    }
}
