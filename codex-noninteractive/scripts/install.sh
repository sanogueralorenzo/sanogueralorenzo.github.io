...
TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/codex-noninteractive-target}"
...
BIN_PATH="$TARGET_DIR/release/codex-noninteractive"
...
TMP_PATH="$DEST_DIR/.tmp-codex-noninteractive-$$"
cp "$BIN_PATH" "$TMP_PATH"
chmod +x "$TMP_PATH"
mv -f "$TMP_PATH" "$DEST_DIR/codex-noninteractive"

echo "Installed CLI: $DEST_DIR/codex-noninteractive"
