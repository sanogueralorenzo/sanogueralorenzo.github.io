...
3. `replay` creates verified promotion evidence and `conversation.observe` promotes one repo-specific precedent.
...
- The second hook injects `prec_webhook_replay_boundary` in `contextBlock`.
- The final report shows one promoted precedent and five events: hook checks, replay evidence, and observed traces.
