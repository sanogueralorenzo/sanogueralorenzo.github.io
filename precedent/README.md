...
- `promote-pending` scans queued `promotionTrials`, runs the existing replay-gated `promotion-trial` command only when the trial is marked `autoExecute` and both replay commands match a strict local allowlist, records started/completed/failed receipts, and skips already completed trials on later runs. `report --json` includes `promotionTrialQueue` so a runtime can see ready, blocked, running, completed, and failed work.
