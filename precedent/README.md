...
- `promote-pending` scans queued `promotionTrials`, runs the existing replay-gated `promotion-trial` command, records started/completed/failed receipts, and skips already completed trials on later runs. `report --json` includes `promotionTrialQueue` so a runtime can see ready, running, completed, and failed work.
