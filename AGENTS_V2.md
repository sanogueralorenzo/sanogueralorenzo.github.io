...
- P09 | install.sh | MUST | Run root `./install.sh`.

## Install Gates
- I01 | install-gate | MUST | Root install gate targets only `codex-auth/**`, `codex-sessions/**`, `codex-remote/**`, `codex-menubar/**` excluding each module README.
- I02 | install-gate | MUST | When I01 is active, run root `./install.sh` as the final validation step.
...
- I05 | install-semantics | MUST | `install` means install+launch for runnable apps, install-only for CLI/scripts.

## Runtime Targets
- R01 | android-behavior-change | MUST | If device available, run `./gradlew :app:installDebug` then `adb shell monkey -p <applicationId> -c android.intent.category.LAUNCHER 1`.
- R02 | mac-app-behavior-change | MUST | If runnable target available, run `cd codex-menubar && ./scripts/install.sh`.
- R03 | cli-behavior-change | MUST | Reinstall affected CLI module via its `scripts/install.sh`.

## Script Determinism
- S01 | cli/script | MUST | Keep execution non-interactive and deterministic; use non-zero exit on errors.
